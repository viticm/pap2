...
HRESULT KAnimationModel::LoadMDL(LPCTSTR szFilePath, float fScale, int nColorChannelTable, int nColorChannel)
{
    ...
    bSameFilePath = _tcscmp(m_ModelData.RLMDL.szFilePath, szFilePath) == 0;

    if (HasLoadedMDL() && !bSameFilePath)
    {
        hr = UnloadMDL();
        KGLOG_COM_PROCESS_ERROR(hr);
    }
    ...
    {
        p3DModel = m_ModelData.RLMDL.p3DModel;
        m_ModelData.RLMDL.p3DModel = NULL;
    }
    else
    {
        ...
        _tcsncpy(m_ModelData.RLMDL.szFilePath, szFilePath, _countof(m_ModelData.RLMDL.szFilePath));
        m_ModelData.RLMDL.szFilePath[_countof(m_ModelData.RLMDL.szFilePath) - 1] = _T('\0');
    }
...
Exit0:
    if (FAILED(hrResult))
    {
        ...
        m_ModelData.RLMDL.szFilePath[0] = _T('\0');
...
Exit0:
    if (FAILED(hrResult))
    {
        KGLogPrintf(KGLOG_ERR, "KAnimationModel::GetBonePosition(%s)\n", szBone);
    }
    return hrResult;
}
