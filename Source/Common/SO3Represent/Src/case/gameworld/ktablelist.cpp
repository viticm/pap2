...
BOOL KTableList::Init()
{
    ...
    m_piNpcAnimationFile = OpenTabFile(szFilePath, "iiiiiiiffifssissssssssssssssssssssssss", KG_FILE_OPEN_NORMAL);
    KGLOG_PROCESS_ERROR(m_piNpcAnimationFile);
...
{
    SAFE_RELEASE(m_piNpcAnimationFile);
...
KRidesEquipModel const* KTableList::GetRidesEquipModel(int nIndex, DWORD dwModelID) const
{
    KRidesEquipModel Found = { dwModelID };

    ASSERT(nIndex > MODEL_RIDES_EQUIP_MANE && nIndex < MODEL_RIDES_EQUIP_COUNT);
    ...
    if (pResult != m_pRidesEquipModel[nIndex] + m_nRidesEquipCount[nIndex] && pResult->dwModelID == dwModelID)
        return pResult;

    KFilePath::TabError("KTableList::GetRidesEquipModel(Index %d, ModelID %u) %s\n", nIndex, dwModelID,
        g_pRL->m_FilePath.GetFilePath(KFilePath::FILE_PATH_RIDES_DIRECTORY, g_szRidesEquip[nIndex]));
    return NULL;
}

KRidesSocketModel const* KTableList::GetRidesSocketModel(int nIndex, DWORD dwModelID) const
{
    KRidesSocketModel Found = { dwModelID };

    ASSERT(nIndex < MODEL_RIDES_SOCKET_COUNT);

    KRidesSocketModel const* pResult = std::lower_bound(m_pRidesSocketModel[nIndex], m_pRidesSocketModel[nIndex] + m_nRidesSocketCount[nIndex], Found);
    if (pResult != m_pRidesSocketModel[nIndex] + m_nRidesSocketCount[nIndex] && pResult->dwModelID == dwModelID)
        return pResult;

    KFilePath::TabError("KTableList::GetRidesSocketModel(Index %d, ModelID %u) %s\n", nIndex, dwModelID,
...
int KTableList::GetNpcAnimationModel(DWORD dwRepresentID, DWORD dwAnimationID, KNpcAnimationModel *pRetRsult)
{
    int nResult = false;
    int nRetCode = false;
    ...
    KGLOG_PROCESS_ERROR(pRetRsult);

    nRetCode = m_piNpcAnimationFile->Search((void *)&Found, KNpcAnimationModel::Compare, (void *)pRetRsult, sizeof(KNpcAnimationModel));
...
Exit0:
    if (!nResult)
    {
        ...
        {
            if (nRoleType >= 0 && nRoleType < _countof(m_RoleTypeData) && 
                nEquipPart >= 0 && nEquipPart < _countof(m_RoleTypeData[nRoleType].EquipSFXData))
            {
                LPCTSTR szFilePath = NULL;

				szFilePath = g_pRL->m_FilePath.GetFilePath(KFilePath::FILE_PATH_PLAYER_EQUIP_SFX_DIRECTORY, 
                    g_szPlayerModelSFXFile[nRoleType][nEquipPart]);
...
KRLPlayerEquipSFXSequence const* KTableList::GetPlayerEquipSFXSequence(
    DWORD dwEquipType,
    ROLE_TYPE nRoleType,
    DWORD dwEquipID,
    DWORD dwSequenceID
...
{
    ASSERT(dwEquipType >= 0 && dwEquipType < perRepresentCount);
    ASSERT(nRoleType >= 0 && nRoleType < rtTotal);
    
    KRLPlayerEquipSFXSequence Found = { dwEquipID, dwSequenceID };
    KRLPlayerEquipSFXSequence* pSequenceModel = m_PlayerEquipSFXSequenceData[dwEquipType][nRoleType].pModel;
    size_t nSequenceCount = m_PlayerEquipSFXSequenceData[dwEquipType][nRoleType].nCount;

    KRLPlayerEquipSFXSequence const* pResult = std::lower_bound(
        pSequenceModel,
        pSequenceModel + nSequenceCount,
        Found
    );

    if (pResult != pSequenceModel + nSequenceCount && pResult->dwEquipID == dwEquipID && pResult->dwSequenceID == dwSequenceID)
        return pResult;

    return NULL;
}

KRLPlayerEquipSFXSequenceIndex const* KTableList::GetPlayerEquipSFXSequenceIndex(DWORD dwEquipType) const
{
    KRLPlayerEquipSFXSequenceIndex Found = { dwEquipType };

    KRLPlayerEquipSFXSequenceIndex const* pResult = std::lower_bound(
        m_pPlayerEquipSFXSequenceIndexModel,
        m_pPlayerEquipSFXSequenceIndexModel + m_nPlayerEquipSFXSequenceIndexCount,
        Found
    );
    if (pResult != m_pPlayerEquipSFXSequenceIndexModel + m_nPlayerEquipSFXSequenceIndexCount &&
        pResult->dwEquipType == dwEquipType)
    {
        return pResult;
    }

    KFilePath::TabError("KTableList::GetPlayerEquipSFXSequenceIndex(dwEquipType %u) %s\n", dwEquipType, 
        g_pRL->m_FilePath.GetFilePath(KFilePath::FILE_PATH_PLAYER_EQUIP_SFX_SEQUENCE_INDEX));
    return NULL;
}

KRLPlayerEquipSFXModelParam const* KTableList::GetPlayerEquipSFXModelParam(DWORD dwSFXID) const
{
    KRLPlayerEquipSFXModelParam Found = { dwSFXID };

    KRLPlayerEquipSFXModelParam const* pResult = std::lower_bound(m_pPlayerEquipSFXModelParamModel, m_pPlayerEquipSFXModelParamModel + m_nPlayerEquipSFXModelParamCount, Found);
    if (pResult != m_pPlayerEquipSFXModelParamModel + m_nPlayerEquipSFXModelParamCount && pResult->dwSFXID == dwSFXID)
        return pResult;

    KFilePath::TabError("KTableList::GetPlayerEquipSFXModelParam(dwSFXID %u) %s\n", dwSFXID, 
        g_pRL->m_FilePath.GetFilePath(KFilePath::FILE_PATH_PLAYER_EQUIP_SFX_MODEL_PARAM));
    return NULL;
}
...
HRESULT KTableList::LoadRidesEquipAndSocketModel()
{
    ...
    KGLOG_PROCESS_ERROR(nValue == (MODEL_RIDES_EQUIP_COUNT - MODEL_RIDES_EQUIP_HEAD));
    ...
    {
        _sntprintf(szKey, _countof(szKey), _T("Item_%d"), nIndex - MODEL_RIDES_EQUIP_HEAD);
    ...
    for (int nIndex = 0; nIndex < MODEL_RIDES_SOCKET_COUNT; ++nIndex)
    {
        ...
        nRetCode = m_RidesSocketTab[nIndex].Load(szFilePath, "iiifsss");
        KGLOG_PROCESS_ERROR(nRetCode);

        m_pRidesSocketModel[nIndex] = (KRidesSocketModel*)m_RidesSocketTab[nIndex].Data(m_nRidesSocketCount[nIndex]);
...
int KTableList::LoadPlayerAllModel()
{
    ...
    // load equip sfx sequence
    nRetCode = LoadPlayerEquipSFXSequence();
    KGLOG_PROCESS_ERROR(nRetCode);

    // Load equip sfx model param  
    {
        szFilePath = g_pRL->m_FilePath.GetFilePath(KFilePath::FILE_PATH_PLAYER_EQUIP_SFX_MODEL_PARAM);
        KGLOG_PROCESS_ERROR(szFilePath[0] != _T('\0'));

        nRetCode = m_PlayerEquipSFXModelParamTab.Load(szFilePath, "isssfs");
        KGLOG_PROCESS_ERROR(nRetCode);

        m_pPlayerEquipSFXModelParamModel = 
            (KRLPlayerEquipSFXModelParam*)m_PlayerEquipSFXModelParamTab.Data(m_nPlayerEquipSFXModelParamCount);
        KGLOG_PROCESS_ERROR(m_pPlayerEquipSFXModelParamModel);
    }
...
int KTableList::LoadPlayerEquipSFXSequence()
{
    int nResult = false;
    int nRetCode = false;
    ...
    szFilePath = g_pRL->m_FilePath.GetFilePath(KFilePath::FILE_PATH_PLAYER_EQUIP_SFX_SEQUENCE_INDEX);
    KGLOG_PROCESS_ERROR(szFilePath[0] != _T('\0'));

    nRetCode = m_PlayerEquipSFXSequenceIndexTab.Load(szFilePath, "issssssss");
    KGLOG_PROCESS_ERROR(nRetCode);

    m_pPlayerEquipSFXSequenceIndexModel = (KRLPlayerEquipSFXSequenceIndex *)m_PlayerEquipSFXSequenceIndexTab.Data(m_nPlayerEquipSFXSequenceIndexCount);
    KGLOG_PROCESS_ERROR(m_pPlayerEquipSFXSequenceIndexModel);

    for (size_t nIndexCount = 0; nIndexCount < m_nPlayerEquipSFXSequenceIndexCount; nIndexCount++)
    {
        KRLPlayerEquipSFXSequenceIndex *pCurrentIndexModel = m_pPlayerEquipSFXSequenceIndexModel + nIndexCount;
        KGLOG_PROCESS_ERROR(pCurrentIndexModel);

        for (size_t nRoleType = rtInvalid; nRoleType < rtTotal; nRoleType++)
        {
            DWORD dwEquipType = pCurrentIndexModel->dwEquipType;
            KRLPlayerEquipSFXSequenceData& rPlayerEquipSFXSequenceData = m_PlayerEquipSFXSequenceData[dwEquipType][nRoleType];

            szFilePath = pCurrentIndexModel->szSFXSequenceFilePaths[nRoleType];
            KGLOG_PROCESS_ERROR(szFilePath[0] != _T('\0'));

            nRetCode = rPlayerEquipSFXSequenceData.Tab.Load(szFilePath, "iisiiiiiiii");
            KGLOG_PROCESS_ERROR(nRetCode);

            rPlayerEquipSFXSequenceData.pModel = 
                (KRLPlayerEquipSFXSequence *)rPlayerEquipSFXSequenceData.Tab.Data(rPlayerEquipSFXSequenceData.nCount);
            KGLOG_PROCESS_ERROR(rPlayerEquipSFXSequenceData.pModel);
        }
    }
...
HRESULT KTableList::LoadPlayerAllAnimationModel()
{
    ...
    for (int nRoleType = 0; nRoleType != rtTotal; ++nRoleType)
    {
        ...
        szFilePath = g_pRL->m_FilePath.GetFilePath(KFilePath::FILE_PATH_PLAYER_DIRECTORY, 
            g_szPlayerAnimationIndexFiles[nRoleType]);
        KGLOG_PROCESS_ERROR(szFilePath);
    ...
    for (int nRoleType = 0; nRoleType != rtTotal; ++nRoleType)
    {
        ...
        szFilePath = g_pRL->m_FilePath.GetFilePath(KFilePath::FILE_PATH_PLAYER_DIRECTORY, 
            g_szPlayerRandomAnimationFiles[nRoleType]);
        KGLOG_PROCESS_ERROR(szFilePath);
    ...
    for (int nRoleType = 0; nRoleType != rtTotal; ++nRoleType)
    {
        ...
        szFilePath = g_pRL->m_FilePath.GetFilePath(KFilePath::FILE_PATH_PLAYER_DIRECTORY, 
            g_szPlayerSequenceAnimationFiles[nRoleType]);
        KGLOG_PROCESS_ERROR(szFilePath);
    ...
    for (int nRoleType = 0; nRoleType != rtTotal; ++nRoleType)
    {
        ...
        szFilePath = g_pRL->m_FilePath.GetFilePath(KFilePath::FILE_PATH_PLAYER_DIRECTORY, 
            g_szPlayerAnimationFiles[nRoleType]);
        KGLOG_PROCESS_ERROR(szFilePath);
    ...
    for (int nRoleType = 0; nRoleType != rtTotal; ++nRoleType)
    {
        ...
        szFilePath = g_pRL->m_FilePath.GetFilePath(KFilePath::FILE_PATH_PLAYER_DIRECTORY, 
            g_szPlayerAnimationAdjustFiles[nRoleType]);
        KGLOG_PROCESS_ERROR(szFilePath);
    ...
    for (int nRoleType = 0; nRoleType != rtTotal; ++nRoleType)
    {
        ...
        szFilePath = g_pRL->m_FilePath.GetFilePath(KFilePath::FILE_PATH_PLAYER_DIRECTORY, 
            g_szPlayerAnimationTerrainAdjustFiles[nRoleType]);
        KGLOG_PROCESS_ERROR(szFilePath);
    ...
    for (int nRoleType = 0; nRoleType != rtTotal; ++nRoleType)
    {
        ...
        szFilePath = g_pRL->m_FilePath.GetFilePath(KFilePath::FILE_PATH_PLAYER_DIRECTORY, 
            g_szPlayerAnimationFusionFiles[nRoleType]);
        KGLOG_PROCESS_ERROR(szFilePath);
