...
static float GetAnimationSpeed(RL_ANIMATION_DOWN nDownType, RL_ADVANCE nAdvance, int nVelocityXY, BOOL bRide)
{
    float fSpeed = 1.0f;
...
float GetAnimationSpeed(CHARACTER_MOVE_STATE nMoveState, float fModelScale, int nVelocityXY, BOOL bOnRide, RL_ANIMATION_DOWN nAnimationDown, RL_ADVANCE nAdvance)
{
    ...
    if (nMoveState == cmsOnWalk ||
	    nMoveState == cmsOnRun ||
        nMoveState == cmsOnSwim)
    {
    ...
    if (abs(fModelScale) > FLT_EPSILON)
        fSpeed /= fModelScale;
...
{
    static float const STAND_PITCH_STEP_SPEED = D3DX_PI / 200.0f;
    static float const MOVING_PITCH_STEP_SPEED = D3DX_PI / 600.0f;

    float fPitchStepSpeed = 0.0f;

    if (nMoveState == cmsOnStand)
        fPitchStepSpeed = STAND_PITCH_STEP_SPEED;
    else
        fPitchStepSpeed = MOVING_PITCH_STEP_SPEED;

    return fPitchStepSpeed;
}

float GetRollStepSpeed(CHARACTER_MOVE_STATE nMoveState)
{
    static float const STAND_ROLL_STEP_SPEED = D3DX_PI / 100.0f;
    static float const MOVING_ROLL_STEP_SPEED = D3DX_PI / 600.0f;

    float fRollStepSpeed = 0.0f;

    if (nMoveState == cmsOnStand)
        fRollStepSpeed = STAND_ROLL_STEP_SPEED;
    else
        fRollStepSpeed = MOVING_ROLL_STEP_SPEED;

    return fRollStepSpeed;
}
