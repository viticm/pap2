...
HRESULT KRLGodCamera::PostUpdate(KRLCameraParam const& RLCameraParam)
{
    return S_OK;
}
...
int KRLGodCamera::UpdateMove(KG_CAMERA_COMMON_INFO* pCameraCommonInfo)
{
    ...
    nRetCode = ComputeForwardMove(pCameraCommonInfo->nControls, &nForwardMove);
    KGLOG_PROCESS_ERROR(nRetCode);

    nRetCode = ComputeStrafeMove(pCameraCommonInfo->nControls, &nStrafeMove);
    KGLOG_PROCESS_ERROR(nRetCode);

    nRetCode = ComputeUpMove(pCameraCommonInfo->nControls, &nUpMove);
    KGLOG_PROCESS_ERROR(nRetCode);
...
int KRLGodCamera::ComputeForwardMove(const int nControls[], int* pnForwardMove)
{
    int nResult = false;
    ...
    if ((nControls[RL_CONTROL_FORWARD] & RL_CONTROL_STATE_ENABLE) || 
...
int KRLGodCamera::ComputeStrafeMove(const int nControls[], int *pnStrafeMove)
{
    int nResult = false;
    ...
    if (nControls[RL_CONTROL_STRAFE_LEFT] & RL_CONTROL_STATE_ENABLE)
        ++nStrafeMove;

    if (nControls[RL_CONTROL_STRAFE_RIGHT] & RL_CONTROL_STATE_ENABLE)
        --nStrafeMove;
...
int KRLGodCamera::ComputeUpMove(const int nControls[], int *pnUpMove)
{
    int nResult = false;
    ...
    if (nControls[RL_CONTROL_UP] & RL_CONTROL_STATE_ENABLE)
        ++nUpMove;

    if (nControls[RL_CONTROL_DOWN] & RL_CONTROL_STATE_ENABLE)
        --nUpMove;
...
int KRLGodCamera::UpdateRotation(KG_CAMERA_COMMON_INFO* pCameraCommonInfo)
{
    ...
    if (
        (pCameraCommonInfo->nControls[RL_CONTROL_OBJECT_STICK_CAMERA] & RL_CONTROL_STATE_ENABLE) == 0 &&
...
HRESULT KRLGodCamera::SetPosition(KG_CAMERA_COMMON_INFO* pCameraCommonInfo)
{
    ...
    hr = pCameraCommonInfo->RLTrackCamera.UpdateCamera(vObjectPosition, m_vCameraPosition, vDirectionC2O, 0.0f, 100.0f);
