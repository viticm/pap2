...
{
	m_dwObjectID = UINT_MAX;
    m_nObjectType = RL_OBJECT_UNKNOWN;
...
HRESULT KRLTarget::AttachSceneObject(DWORD dwObject, RL_OJBECT nObjectType)
{
	HRESULT hr = E_FAIL;
	HRESULT hrResult = E_FAIL;
...
Exit0:
    if (FAILED(hrResult))
    {
	    KGLogPrintf(KGLOG_ERR, "KActionTarget::AttachSceneObject(%u, %u)\n", dwObject, nObjectType);
    }
	return hrResult;
}

HRESULT KRLTarget::DetachSceneObject(DWORD dwObject, RL_OJBECT nObjectType)
{
	Reset();
...
float KRLTarget::GetCharcterScale() const
{
	...
	pRLCharacter = GetRLCharacter(m_dwObjectID);
