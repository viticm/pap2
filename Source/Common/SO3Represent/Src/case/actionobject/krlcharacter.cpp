...
, m_nVehicleTrack(0)
{
    ...
    m_RenderData.Init(this, CHARACTER_EQUIP_COUNT, CHARACTER_SOCKET_COUNT, CHARACTER_SFX_TOTAL_COUNT);
    ...
    memset(m_nPlayerEquipSFXSequence, 0, sizeof(m_nPlayerEquipSFXSequence));
    m_nEquipSFXSequenceModelIndex = 0;
}
...
int KRLCharacter::Init(KCharacter* pCharacter, KRLScene* pRLScene, KRLCharacterFrameData* pFrameData)
{
    int nRetCode = false;
    int nResult = false;
...
{   
    ...
    *ppRLRides = NULL;
    ...
    hr = pRLRides->m_RenderData.SetMDLVisible(FALSE);
    KGLOG_COM_CHECK_ERROR(hr);
...
HRESULT KRLCharacter::UnloadTitleMark()
{
	...
	m_dwTitleMarkEffect = RL_GLOBAL_EFFECT_NONE;
...
int KRLCharacter::PlaySound(int nType, LPCTSTR pszFilePath, float fMinDistance, float fMaxDistance, PDWORD pdwSoundID)
{
    ...
    KG_PROCESS_SUCCESS(!g_pRL->m_bActivated);
...
int KRLCharacter::PlayAnimation(DWORD dwAnimationID, RL_ANIMATION_STATE nState, RL_ANIMATION_UP nUpState, RL_ANIMATION_DOWN nDownState, int nOffsetTime, void* pUserdata)
{
    ...
    KG_PROCESS_SUCCESS(dwAnimationID == 0);

    nRetCode = FillRLAdjustAnimationParam(&Param, m_pFrameData->m_Current.bSheathFlag);
    KGLOG_PROCESS_ERROR(nRetCode);
...
int KRLCharacter::PrepareCastSkill(KSkillCasterParam const& SkillCasterParam)
{
    int nRetCode = false;
    int nResult = false;
    ...
    KG_PROCESS_SUCCESS(SkillCasterParam.dwPrepareAnimationID == 0);

    if (m_nUpBodyState == RL_ANIMATION_UP_SKILL_CHANNEL && m_pFrameData->m_Current.nOTAction != otActionSkillChannel ||
        m_nUpBodyState == RL_ANIMATION_UP_SKILL_PREPARE && m_pFrameData->m_Current.nOTAction != otActionSkillPrepare ||
        m_nUpBodyState == RL_ANIMATION_UP_CAST_SKILL ||
        m_dwSerialAnimationID != 0)
    {
    ...
    nRetCode = PlayAnimation(SkillCasterParam.dwPrepareAnimationID, RL_ANIMATION_STATE_IMPULSE,
...
int KRLCharacter::CastSkill(KSkillCasterParam const& SkillCasterParam, DWORD dwSkillID, DWORD dwSkillLevel, DWORD dwBulletID)
{
    ...
    nRetCode = PlayAnimation(dwAnimationID, RL_ANIMATION_STATE_IMPULSE, RL_ANIMATION_UP_CAST_SKILL, RL_ANIMATION_DOWN_UNKNOWN, 0, (void*)(UINT_PTR)dwBulletID);
    KGLOG_PROCESS_ERROR(nRetCode);
...
HRESULT KRLCharacter::OnMotionNotify(IKG3DAnimationController* pController, DWORD dwMotionInfo, TagMotionCallBackExtraInfo *pMotionCallBackExtraInfo)
{
    ...
    case MTBT_EQUIPSFX:
        {
            TagMoitonCBExtraInfoEquipSFX* pTagMoitonCBExtraInfoEquipSFX = NULL;
            DWORD dwEquipType = 0;
            DWORD dwSequenceID = 0;

            pTagMoitonCBExtraInfoEquipSFX = reinterpret_cast<TagMoitonCBExtraInfoEquipSFX*>(pMotionCallBackExtraInfo);

            dwEquipType = pTagMoitonCBExtraInfoEquipSFX->dwEquipType;
            dwSequenceID = pTagMoitonCBExtraInfoEquipSFX->dwSfxID;

            nRetCode = PlayEquipSFXSequence(dwEquipType, dwSequenceID);
            KGLOG_PROCESS_ERROR(nRetCode);
        }
        break;
    default:
...
int KRLCharacter::UpdateAnimationDown(RL_ANIMATION_DOWN nAnimationDown, RL_ADVANCE nAdvance, 
                                      BOOL bSlip, RL_FIGHT_FLAG nFightFlag,
                                      DWORD dwSkillBuffs[SKILL_BUFF_PART_COUNT])
{
    ...
    if (nAnimationDown == RL_ANIMATION_DOWN_STAND)
    {
        if (m_EquipResource.nObject == RL_OBJECT_PLAYER)
        {
            if (m_nUpBodyState == RL_ANIMATION_UP_SKILL_CHANNEL && m_pFrameData->m_Current.nOTAction != otActionSkillChannel ||
                m_nUpBodyState == RL_ANIMATION_UP_SKILL_PREPARE && m_pFrameData->m_Current.nOTAction != otActionSkillPrepare ||
                m_bSlip != bSlip ||
                m_nFightFlag != nFightFlag && m_nFightFlag && m_nUpBodyState != RL_ANIMATION_UP_CAST_SKILL)
            {
                nRetCode = ResetAnimation();
                KGLOG_PROCESS_ERROR(nRetCode);
            }
    ...
    if ((nAnimationDown == RL_ANIMATION_DOWN_JUMP || nAnimationDown == RL_ANIMATION_DOWN_DOUBLE_JUMP) && 
        ...
        m_pFrameData->m_Current.bAboveWater && 
        m_pFrameData->m_Current.nZ > m_pFrameData->m_Previous.nZ && 
        m_pFrameData->m_Current.nZ * m_pFrameData->m_Previous.nZ < 0)
    {
    ...
    {
        nRetCode = UpdateAnimation(nAnimationDown, nAnimationState, nAdvance, bSlip, nFightFlag, dwSkillBuffs);
        KGLOG_PROCESS_ERROR(nRetCode);
    }
...
int KRLCharacter::UpdateAnimationUp(RL_ANIMATION_UP nAnimationUp)
{
    int nRetCode = false;
    int nResult = false;
    RL_ANIMATION_UP nPrevAnimationUp = RL_ANIMATION_UP_UNKNOWN;

    nPrevAnimationUp = m_nUpBodyState;
    m_nUpBodyState = nAnimationUp;

    if (nAnimationUp == RL_ANIMATION_UP_NONE)
    {
        if (nPrevAnimationUp == RL_ANIMATION_UP_PICKING_DOODAD || 
            nPrevAnimationUp == RL_ANIMATION_UP_RECIPE_PREPARE ||
            nPrevAnimationUp == RL_ANIMATION_UP_PICK_PREPARE || 
            nPrevAnimationUp == RL_ANIMATION_UP_CUSTOM_PREPARE || 
            nPrevAnimationUp == RL_ANIMATION_UP_CUSTOM_CHANNEL ||
            nPrevAnimationUp == RL_ANIMATION_UP_SKILL_PREPARE ||
            nPrevAnimationUp == RL_ANIMATION_UP_SKILL_CHANNEL)
        {
            nRetCode = ResetAnimation();
            KGLOG_PROCESS_ERROR(nRetCode);
        }
...
static int UpdateRemoteVechicleVisible(BOOL bVisible)
{
    ...
    for (KRLScenePtrArray::iterator itScene = RLScenePtrArray.begin(); itScene != RLScenePtrArray.end(); ++itScene)
    {
        ...
        for (KRLRemoteCharacterMgr::KPtrMap::iterator it = Characters.begin(); it != Characters.end(); ++it)
        {
            ...
            hr = pRLRemoteCharacter->m_RLCharacter.m_pRLVehicle->m_RenderData.SetMDLVisible(bVisible);
            KGLOG_COM_PROCESS_ERROR(hr);
        }
...
int KRLCharacter::UpdateVehicleVisible()
{
    ...
    else
    {
        ...
        hr = m_pRLVehicle->m_RenderData.SetMDLVisible(bVisible);
        KGLOG_COM_PROCESS_ERROR(hr);
    }
...
int KRLCharacter::UpdateBalloon()
{
    ...
    KGLOG_PROCESS_ERROR(m_pFrameData);
    KGLOG_PROCESS_ERROR(m_pFrameData->m_pCharacter);

    if (!IS_PLAYER(GetObjectID()))
    {
        pNpc = (KNpc*)m_pFrameData->m_pCharacter;
    ...
    hr = m_RenderData.SetCaption(3, m_pFrameData->m_pCharacter->m_szName);
    KGLOG_COM_PROCESS_ERROR(hr);
    ...
    if (bCanSeeLife && g_pRL->m_Option.bHeadSFXEnabled[nCharacterType][CHARACTER_SFX_LIFE - CHARACTER_SFX_BBOX_BALLOON_BEGIN]) 
    {
        ...
        hr = m_RenderData.SetPercentage(dwColor, m_pFrameData->m_Current.fLifePercentage);
        KGLOG_COM_PROCESS_ERROR(hr);
    }
    else
    {
	...
	// Tong
	if (IS_PLAYER(GetObjectID()) && g_pRL->m_Option.bHeadSFXEnabled[nCharacterType][CHARACTER_SFX_TONG - CHARACTER_SFX_BBOX_BALLOON_BEGIN])
	{
		KPlayer* pPlayer = (KPlayer*)m_pFrameData->m_pCharacter;
    ...
    {
        if (m_pFrameData->m_pCharacter->m_szTitle && m_pFrameData->m_pCharacter->m_szTitle[0] != '\0')
        {
			_sntprintf(szCaption, _countof(szCaption), _T("<%s>"), m_pFrameData->m_pCharacter->m_szTitle);
            szCaption[_countof(szCaption) - 1] = _T('\0');
    ...
    fCaptionFloorSpace = g_pRL->m_TableList.GetNumber(RL_NUMBER_CAPTION_FLOOR_SPACE_HEIGHT);
...
int KRLCharacter::UpdateTitleMarkPosition()
{
    ...
    nRetCode = GetCharacterQuestMarkOrSkillBuffHeight(m_pFrameData->m_pCharacter, fDelta);
    KGLOG_PROCESS_ERROR(nRetCode);
...
int KRLCharacter::UpdateModel(KRLCharacterEquipResource const& EquipResource, BOOL bForceUpdate)
{
    int nRetCode = false;
    int nResult = false;
    int nLoadModel = false;

    nRetCode = UnloadModel();
    KGLOG_PROCESS_ERROR(nRetCode);

    nRetCode = LoadModel(EquipResource, bForceUpdate);
    KGLOG_PROCESS_ERROR(nRetCode);

    nLoadModel = true;

    if (m_pFrameData->m_Current.bOnRide)
    {
        nRetCode = UpdateRide();
        KGLOG_PROCESS_ERROR(nRetCode);
    }
...
int KRLCharacter::SetSelectable(BOOL bSelectable)
{
    ...
    if (m_pRLHorse == NULL && m_pRLVehicle == NULL)
    {
...
int KRLCharacter::LoadModel(KRLCharacterEquipResource const& EquipResource, BOOL bForceUpdate)
{
    int nRetCode = false;
    int nResult = false;
    HRESULT hr = E_FAIL;
    ...
    {
        nRetCode = LoadSocket(nIndex, EquipResource.SocketParams[nIndex], EquipResource.nColorChannelTable);
        KGLOG_CHECK_ERROR(nRetCode);
    }
...
int KRLCharacter::LoadSocketByAniamtion(DWORD dwAnimationID)
{
    ...
    for (int nIndex = 0; nIndex != CHARACTER_SOCKET_COUNT; ++nIndex)
    {
        ...
        {
            nRetCode = LoadSocket(nIndex, m_EquipResource.SocketParams[nIndex], m_EquipResource.nColorChannelTable);
            KGLOG_CHECK_ERROR(nRetCode);
        }
        else
        {
            nRetCode = LoadSocket(nIndex, AnimationSocketResource[nIndex], m_EquipResource.nColorChannelTable);
            KGLOG_CHECK_ERROR(nRetCode);
        }
    }
...
int KRLCharacter::LoadSocket(int nIndex, KRLSocketResource const SocketParam, int nColorChannelTable)
{
    int nResult = false;
    HRESULT hr = E_FAIL;
...
DWORD KRLCharacter::GetAnimationID(RL_ANIMATION_TABLE nAnimationTable, BOOL bSheathFlag) const
{
    ...
    nRetCode = GetCharacterAnimationIndex(m_EquipResource, 
        (RL_SCHOOL)m_pFrameData->m_Current.nSchoolID, 
...
RL_ANIMATION_TABLE KRLCharacter::GetAnimationTable(RL_ANIMATION_DOWN nDownType, RL_ADVANCE nAdvance, RL_FIGHT_FLAG nFightFlag) const
{
    ...
    Param.bSheathFlag = m_pFrameData->m_Current.bSheathFlag;
    Param.bSlip = m_pFrameData->m_Current.bSlip;
                                  ...
                                  DWORD dwSkillBuffs[SKILL_BUFF_PART_COUNT])
{
    ...
    if (m_nDownBodyState != RL_ANIMATION_DOWN_NONE && m_nDownBodyState != nAnimationDown)
    {
        ...
        {
            dwAnimationID = GetAnimationID(nAnimationTable, m_pFrameData->m_Current.bSheathFlag);
    ...
    if (IS_PLAYER(GetObjectID()))
        bImpluseAnimation = m_pDownBodyCurrentAnimationTrack->nState == RL_ANIMATION_STATE_IMPULSE;
    else
        bImpluseAnimation = m_DownBodyLowAnimationTrack.nState == RL_ANIMATION_STATE_IMPULSE;

    if (bImpluseAnimation || 
        m_nDownBodyState != nAnimationDown || 
        m_AnimationStatus.nAdvance != nAdvance ||
        m_bSlip != bSlip ||
        m_nFightFlag != nFightFlag && m_nFightFlag ||
        memcmp(m_dwSkillBuffs, dwSkillBuffs, sizeof(m_dwSkillBuffs)) != 0)
    {
        ...
        {
            KG_PROCESS_SUCCESS(m_pRLVehicle == NULL);
...
void KRLCharacter::TurnPlayerFaceBegin(float fDeltaDirection)
{
    ...
    static float const DIRECTION_EPSLON_MIN = -D3DX_PI / 180.0f;
    static float const DIRECTION_EPSLON_MAX = D3DX_PI / 180.0f;
    ...
    KG_PROCESS_SUCCESS(
        (fDeltaDirection < DIRECTION_EPSLON_MAX) && 
        (fDeltaDirection > DIRECTION_EPSLON_MIN)
    );
                                      ...
                                      int nAnimationState, int nOffsetTime, void* pUserdata)
{
    ...
    if (g_pRL->m_CurrentTraceOption.bTracePlayerAnimation)
    {
        ...
        printf("[RL] Try PlaySplitAnimation(jointtype:%d, animationid:%d, state:%s, up/down state:%s)\n", 
            nJointType, 
            dwAnimationID, 
    ...
    {
        if (m_pFrameData->m_Current.nMoveState == cmsOnStand)
        {
            dwAnimationTrackID = GetAnimationTrackID(ANI_JOINT_TOP, Animation.dwKindID);
        }
        else
        {
            dwAnimationTrackID = GetAnimationTrackID(nJointType, Animation.dwKindID);
    ...
    if (nJointType == ANI_JOINT_TOP)
    {
        if (dwAnimationTrackID > 0)
        {
            ...
            UpdateWeaponPosition(Animation.nSheathSocket, m_pFrameData->m_Current.bSheathFlag);
        }
        else
        {
...
void KRLCharacter::PlayWholeAnimation(KRLAniInfo Info)
{
    ...
    if (dwTrackID)
    {
        ...
        m_DownBodyHighAnimationTrack.nAnimationDownType = m_nDownBodyState;
        m_DownBodyHighAnimationTrack.dwAnimationID = Info.dwAnimationID;
        m_DownBodyHighAnimationTrack.dwKindID = Animation.dwKindID;
        m_DownBodyHighAnimationTrack.dwTrackID = dwTrackID;
        m_DownBodyHighAnimationTrack.nState = Info.nState;
    ...
    else
    {
        ...
        m_DownBodyLowAnimationTrack.nAnimationDownType = m_nDownBodyState;
        m_DownBodyLowAnimationTrack.dwAnimationID = Info.dwAnimationID;
        m_DownBodyLowAnimationTrack.dwKindID = Animation.dwKindID;
        m_DownBodyLowAnimationTrack.dwTrackID = dwTrackID;
        m_DownBodyLowAnimationTrack.nState = Info.nState;
    ...
    else
    {
        ...
        if (m_nDownBodyState == RL_ANIMATION_DOWN_NONE)
        {
            m_nDownBodyState = GetRLAnimationDown(
                m_pFrameData->m_Current.nMoveState, 
                m_pFrameData->m_Current.nOTAction,
                m_pFrameData->m_Current.nJumpCount,
        ...
        {
            if (m_pFrameData->m_Current.nMoveState == cmsOnSwim || 
                m_pFrameData->m_Current.nMoveState == cmsOnFloat)
            {
        ...
        if (m_EquipResource.nObject == RL_OBJECT_NPC && m_nDownBodyState == RL_ANIMATION_DOWN_STAND)
        {
            // npc idle 的时候循环播放
            nAnimationState = RL_ANIMATION_STATE_HOLD_LOOP;
        }
...
int KRLCharacter::UpBodyHighTrackAnimationFinish()
{
    ...
    case RL_ANIMATION_UP_CAST_SKILL:
    ...
    case RL_ANIMATION_UP_BEHIT:
        ...
        if (m_pUpBodyCurrentAnimationTrack->dwAnimationID)
        {
            ...
            UpdateWeaponPosition(m_pUpBodyCurrentAnimationTrack->nSheathSocket, m_pFrameData->m_Current.bSheathFlag);
        }
...
int KRLCharacter::DownBodyLowTrackAnimationFinish()
{
    ...
    // low track
    nOffsetTime = GetAnimationPlayTime(m_AnimationStatus.SplitStatus.UpLowTrackParam.fStartTime);

    nAnimationTable = GetAnimationTable(m_nDownBodyState, m_AnimationStatus.nAdvance, m_nFightFlag);
    dwAnimationID = GetAnimationID(nAnimationTable, m_pFrameData->m_Current.bSheathFlag);

    if (m_DownBodyLowAnimationTrack.nState == RL_ANIMATION_STATE_IMPULSE)
    {
        dwAnimationID = m_UpBodyLowAnimationTrack.dwAnimationID;
        nAnimationState = m_UpBodyLowAnimationTrack.nState;
    }

    nRetCode = PlaySplitAnimation(ANI_JOINT_BOTTOM, dwAnimationID, nAnimationState, RL_ANIMATION_DOWN_UNKNOWN, nOffsetTime);
    KGLOG_PROCESS_ERROR(nRetCode);
...
int KRLCharacter::WholeBodyLowTrackAnimationFinish()
{
    ...
    case RL_ANIMATION_STATE_HOLD_LOOP:
        if (m_nDownBodyState == RL_ANIMATION_DOWN_STAND || m_nDownBodyState == RL_ANIMATION_DOWN_NONE)
        {
            memset(&m_UpBodyLowAnimationTrack, 0, sizeof(m_UpBodyLowAnimationTrack));
...
int KRLCharacter::FillRLAdjustAnimationParam(KRLAdjustAnimationParam* pParam, BOOL bSheathFlag) const
{
    ...
    pParam->bAboveWater = m_pFrameData->m_Current.bAboveWater;
    pParam->bJump = m_pFrameData->m_Current.nMoveState == cmsOnJump || m_pFrameData->m_Current.nMoveState == cmsOnSwimJump; // TODO: error
    pParam->bRide = m_pRLHorse != NULL;
    pParam->bSpeedup = m_pFrameData->m_Current.nVelocityXY >= g_pRL->m_TableList.GetNumber(RL_NUMBER_CHARACTER_RIDE_RUN_FASTER_SPEED);
    ...
    memcpy(pParam->dwSkillBuffs, m_pFrameData->m_Current.dwSkillBuffs, sizeof(pParam->dwSkillBuffs));
...
int KRLCharacter::PlayEquipSFXSequence(DWORD dwEquipType, DWORD dwSequenceID)
{
    int nResult = false;
    int nRetCode = false;
    ...
    DWORD dwSFXID = 0;
    KRLPlayerEquipSFXSequenceIndex const* pSequenceIndexModel = NULL;
    KRLPlayerEquipSFXSequence const* pSequenceModel = NULL;
    KRLPlayerEquipSFXModelParam const* pSFXModelParam = NULL;
    KRLSFXParam SFXParam;

    KGLOG_PROCESS_ERROR(dwEquipType >= 0 && dwEquipType < perRepresentCount);
    KGLOG_PROCESS_ERROR(dwSequenceID >= 0 && dwSequenceID < EQUIP_SFX_SEQUENCE_COUNT);
    KG_PROCESS_SUCCESS(m_EquipResource.nObject != RL_OBJECT_PLAYER);

    nRoleType = m_EquipResource.Player.nRoleType;
    dwEquipID = m_EquipResource.Player.dwRepresentIDs[dwEquipType];

    pSequenceIndexModel = g_pRL->m_TableList.GetPlayerEquipSFXSequenceIndex(dwEquipType);
    KGLOG_PROCESS_ERROR(pSequenceIndexModel);

    pSequenceModel = g_pRL->m_TableList.GetPlayerEquipSFXSequence(dwEquipType, nRoleType, dwEquipID, dwSequenceID);
    KG_PROCESS_SUCCESS(pSequenceModel == NULL);

    if (pSequenceModel->dwSFXIDs[m_nPlayerEquipSFXSequence[dwEquipType][dwSequenceID]] == 0)
    {
        m_nPlayerEquipSFXSequence[dwEquipType][dwSequenceID] = 0;
    }

    dwSFXID = pSequenceModel->dwSFXIDs[m_nPlayerEquipSFXSequence[dwEquipType][dwSequenceID]];

    m_nPlayerEquipSFXSequence[dwEquipType][dwSequenceID] = 
        (m_nPlayerEquipSFXSequence[dwEquipType][dwSequenceID] + 1) % RL_PLAYER_EQUIP_SEQUENCE_SFX_COUNT;

    pSFXModelParam = g_pRL->m_TableList.GetPlayerEquipSFXModelParam(dwSFXID);
    KGLOG_PROCESS_ERROR(pSFXModelParam);

    SFXParam.dwPlayType = ENU_ANIMATIONPLAY_ONCE;
    SFXParam.fScale = pSFXModelParam->fScale;
    SFXParam.fSpeed = 1.0f;
    SFXParam.szFilePath = pSFXModelParam->szSFXFilePath;

    if (pSFXModelParam->szBoneNameLeft[0] == _T('\0') && pSFXModelParam->szBoneNameRight[0] == _T('\0'))
    {
        m_nEquipSFXSequenceModelIndex = (m_nEquipSFXSequenceModelIndex + 1) % CHARACTER_SEQUENCE_SFX_COUNT;
        SFXParam.szBoneName = _T("");

        hr = m_RenderData.LoadMDLSFX(CHARACTER_SEQUENCE_SFX_BEGIN + m_nEquipSFXSequenceModelIndex, SFXParam);
        KGLOG_COM_PROCESS_ERROR(hr);
    }

    if (pSFXModelParam->szBoneNameLeft[0] != _T('\0'))
    {
        m_nEquipSFXSequenceModelIndex = (m_nEquipSFXSequenceModelIndex + 1) % CHARACTER_SEQUENCE_SFX_COUNT;
        SFXParam.szBoneName = pSFXModelParam->szBoneNameLeft;

        hr = m_RenderData.LoadMDLSFX(CHARACTER_SEQUENCE_SFX_BEGIN + m_nEquipSFXSequenceModelIndex, SFXParam);
        KGLOG_COM_PROCESS_ERROR(hr);
    }

    if (pSFXModelParam->szBoneNameRight[0] != _T('\0'))
    {
        m_nEquipSFXSequenceModelIndex = (m_nEquipSFXSequenceModelIndex + 1) % CHARACTER_SEQUENCE_SFX_COUNT;
        SFXParam.szBoneName = pSFXModelParam->szBoneNameRight;
