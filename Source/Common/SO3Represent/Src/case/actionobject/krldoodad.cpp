...
static RL_DOODAD_ANIMATION GetDoodadAnimationID(RL_DOODAD_STATE nPreviousState, RL_DOODAD_STATE nCurrentState)
{
    ...
    case RL_DOODAD_STATE_NONE:
        ...
        {
        case RL_DOODAD_STATE_NONE:
            nDoodadAnimation = RL_DOODAD_ANIMATION_IDLE;
            break;
    ...
    case RL_DOODAD_STATE_OPERATION:
        ...
        case RL_DOODAD_STATE_LEAVE_SCENE:
            break;
        case RL_DOODAD_STATE_IDLE:
            nDoodadAnimation = RL_DOODAD_ANIMATION_OPERATION_TO_IDLE;
            break;
        case RL_DOODAD_STATE_OPERATION:
            nDoodadAnimation = RL_DOODAD_ANIMATION_OPERATION;
            break;
    ...
    case RL_DOODAD_STATE_AFTER_OPERATION:
        ...
        case RL_DOODAD_STATE_LEAVE_SCENE:
            break;
        case RL_DOODAD_STATE_IDLE:
            nDoodadAnimation = RL_DOODAD_ANIMATION_AFTER_OPERATION_TO_IDLE;
            break;
        case RL_DOODAD_STATE_OPERATION:
...
int KRLDoodad::UpdateState(RL_DOODAD_STATE nState, BOOL bForceUpdate)
{
    ...
    else
    {
        if (bForceUpdate)
        {
            ...
            if (pRLDoodadModel->Animations[nAnimation].szMeshFilePath[0] == _T('\0'))
                nAnimation = GetDoodadAnimationID(RL_DOODAD_STATE_NONE, nState);
...
HRESULT KRLDoodad::UpdateLootList()
{
    ...
    if (
        m_FrameData.m_pDoodad->m_eKind == dkCorpse ||
        m_FrameData.m_pDoodad->m_eKind == dkNpcDrop
    )
    {
...
int KRLDoodad::PlayDoodadAnimation(RL_DOODAD_ANIMATION nAni, KRLDoodadModel const& RLDoodadModel)
{
    ...
    {
        if (m_RenderData.HasLoadedMDLSFX(DOODAD_SFX_MAIN))
        {
            hr = m_RenderData.UnloadMDLSFX(DOODAD_SFX_MAIN);
            KGLOG_COM_PROCESS_ERROR(hr);
        }
    }
...
int KRLDoodad::PlayCorpseAnimation(DWORD dwAnimationID)
{
    ...
    nRetCode = g_pRL->m_TableList.GetNpcAnimationModel(m_dwRepresentID, dwAnimationID, &NpcAnimation);
	KG_PROCESS_SUCCESS(!nRetCode);

	MonoAnimationParam.dwPlayType = AnimationPlayTypeRLTo3D((RL_ANIMATION_PLAY_TYPE)NpcAnimation.dwAnimationType);
	MonoAnimationParam.fSpeed = NpcAnimation.fAnimationSpeed;
    MonoAnimationParam.nOffsetTime = ANIMATION_SEEK_OFFSET_END;
    MonoAnimationParam.dwTweenTime = 0;
    MonoAnimationParam.pUserdata = NULL;
	MonoAnimationParam.szFilePath = NpcAnimation.szAnimationFile;
    MonoAnimationParam.fStartTime = g_pRL->m_fTime;

    dwTrackType = g_pRL->m_TableList.GetNPCAnimationTrackType(NpcAnimation.dwKindID);
    if (dwTrackType > 0)
        MonoAnimationParam.nTrackType = ANICTL_SECONDARY;
    else
        MonoAnimationParam.nTrackType = ANICTL_PRIMARY;

	SFXParam.dwPlayType = AnimationPlayTypeRLTo3D((RL_ANIMATION_PLAY_TYPE)NpcAnimation.dwSFXType);
	SFXParam.fSpeed = NpcAnimation.fSFXSpeed;
	SFXParam.fScale = NpcAnimation.fSFXScale;
	SFXParam.szFilePath = NpcAnimation.szSFXFile;
	SFXParam.szBoneName = NpcAnimation.szSFXBoneName;

    hr = m_RenderData.UnloadAllSocket();
    KGLOG_COM_PROCESS_ERROR(hr);

	hr = LoadCorpseSocket(&NpcAnimation);
    KGLOG_COM_PROCESS_ERROR(hr);
...
int KRLDoodad::Update(double fTime, double fTimeLast, DWORD dwGameLoop, BOOL bFrame)
{
	...
	if (bFrame)
    {
        ...
        // 错开更新
        if (m_bOnFloor && (dwGameLoop % 0xFF) == (m_FrameData.m_pDoodad->m_dwID & 0xFF))
        {
...
HRESULT KRLDoodad::OnPlayAnimationFinish(IKG3DAnimationController* pController)
{
    ...
    KG_PROCESS_SUCCESS(m_FrameData.m_pDoodad->m_eKind == dkCorpse);
