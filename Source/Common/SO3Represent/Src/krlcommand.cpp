...
static int ExecuteTraceCommand(LPCTSTR szCommand)
{
    ...
    if (_tcsicmp(szCommand, _T("trace camera")) == 0)
    {
        g_pRL->m_TraceOption.bTraceCamera = !g_pRL->m_TraceOption.bTraceCamera;
        g_pRL->m_CurrentTraceOption.bTraceCamera = g_pRL->m_TraceOption.bTraceCamera;

        goto Exit1;
    }

    if (_tcsicmp(szCommand, _T("trace target")) == 0)
    {
        g_pRL->m_TraceOption.bTraceTarget = !g_pRL->m_TraceOption.bTraceTarget;
        g_pRL->m_CurrentTraceOption.bTraceTarget = g_pRL->m_TraceOption.bTraceTarget;

        goto Exit1;
    }

    if (_tcsicmp(szCommand, _T("trace frame interval")) == 0)
    {
        g_pRL->m_TraceOption.bTraceFrameInterval = !g_pRL->m_TraceOption.bTraceFrameInterval;
        g_pRL->m_CurrentTraceOption.bTraceFrameInterval = g_pRL->m_TraceOption.bTraceFrameInterval;

        goto Exit1;
    }

    if (_tcsicmp(szCommand, _T("trace prefetch character position")) == 0)
    {
        g_pRL->m_TraceOption.bTracePrefetchCharacterPosition = !g_pRL->m_TraceOption.bTracePrefetchCharacterPosition;
        g_pRL->m_CurrentTraceOption.bTracePrefetchCharacterPosition = g_pRL->m_TraceOption.bTracePrefetchCharacterPosition;

        goto Exit1;
    }

    if (_tcsicmp(szCommand, _T("trace frame time")) == 0)
    {
        g_pRL->m_TraceOption.bTraceFrameTime = !g_pRL->m_TraceOption.bTraceFrameTime;
        g_pRL->m_CurrentTraceOption.bTraceFrameTime = g_pRL->m_TraceOption.bTraceFrameTime;

        goto Exit1;
    }

    if (_tcsicmp(szCommand, _T("trace character control")) == 0)
    {
        g_pRL->m_TraceOption.bTraceCharacterControl = !g_pRL->m_TraceOption.bTraceCharacterControl;
        g_pRL->m_CurrentTraceOption.bTraceCharacterControl = g_pRL->m_TraceOption.bTraceCharacterControl;

        goto Exit1;
    }
    
    if (_tcsicmp(szCommand, _T("trace msg")) == 0)
    {
        g_pRL->m_TraceOption.bTraceMsg = !g_pRL->m_TraceOption.bTraceMsg;
        g_pRL->m_CurrentTraceOption.bTraceMsg = g_pRL->m_TraceOption.bTraceMsg;

        goto Exit1;
    }
    
    if (_tcsicmp(szCommand, _T("trace character")) == 0)
    {
        g_pRL->m_TraceOption.bTraceCharacter = !g_pRL->m_TraceOption.bTraceCharacter;
        g_pRL->m_CurrentTraceOption.bTraceCharacter = g_pRL->m_TraceOption.bTraceCharacter;
...
static int ExecuteDebugCommand(LPCTSTR szCommand)
{
    int nRetCode = false;
    int nResult = false;

    if (_tcsicmp(szCommand, _T("disable animation")) == 0)
    {
        g_pRL->m_DebugOption.bDisableAnimation = !g_pRL->m_DebugOption.bDisableAnimation;
        g_pRL->m_CurrentDebugOption.bDisableAnimation = g_pRL->m_DebugOption.bDisableAnimation;

        goto Exit1;
    }

    if (_tcsicmp(szCommand, _T("disable remote character interpolate")) == 0)
    {
        g_pRL->m_DebugOption.bDisableRemoteCharacterInterpolate = !g_pRL->m_DebugOption.bDisableRemoteCharacterInterpolate;
        g_pRL->m_CurrentDebugOption.bDisableRemoteCharacterInterpolate = g_pRL->m_DebugOption.bDisableRemoteCharacterInterpolate;
...
static int ExecuteResourceLimitCommand(LPCTSTR szCommand)
{
    int nRetCode = false;
    int nResult = false;

    if (_tcsicmp(szCommand, _T("limit resource")) == 0)
    {
        g_pRL->m_ResourceLimit.bEnable = !g_pRL->m_ResourceLimit.bEnable;

        goto Exit1;
    }
    if (_tcsncmp(szCommand, _T("limit character"), _tcslen(_T("limit character"))) == 0)
    {
        int nCount = 0;

        nRetCode = _stscanf_s(szCommand, _T("limit character %u"), &nCount) == 1;
        KGLOG_PROCESS_ERROR(nRetCode);

        g_pRL->m_ResourceLimit.nCharacterMaxVisibleCount = nCount;
...
static int ExecuteProfileCommand(LPCTSTR szCommand, void* pResult)
{
    int nRetCode = false;
    int nResult = false;

    // profile
    if (_tcsicmp(szCommand, _T("profile")) == 0)
    {
        g_pRL->m_Profile.bEnable = !g_pRL->m_Profile.bEnable;

        goto Exit1;
    }

    if (_tcsicmp(szCommand, _T("reset profile")) == 0)
    {
        nRetCode = ResetProfile(&g_pRL->m_Profile);
        KGLOG_PROCESS_ERROR(nRetCode);
...
static int SetDoodadState(DWORD dwDoodad, int nState)
{
    int nRetCode = false;
    int nResult = false;
    ...
    KGLOG_PROCESS_ERROR(nState >= RL_DOODAD_STATE_NONE && nState < RL_DOODAD_STATE_COUNT);

    for (KRLScenePtrArray::iterator itScene = RLScenePtrArray.begin(); itScene != RLScenePtrArray.end(); ++itScene)
    {
        pRLScene = *itScene;
        
        pRLDoodad = pRLScene->m_DoodadMgr.Lookup(dwDoodad);
        if (pRLDoodad == NULL)
            continue;

        nRetCode = pRLDoodad->SetState((RL_DOODAD_STATE)nState);
        KGLOG_PROCESS_ERROR(nRetCode);
    }
...
static int ExecuteDoodadCommand(LPCTSTR szCommand, void* pResult)
{
    int nRetCode = false;
    int nResult = false;

    if (_tcsncmp(szCommand, _T("set doodad state"), _tcslen(_T("set doodad state"))) == 0)
    {
        DWORD dwDoodad = 0;
        int nState = 0;

        nRetCode = _stscanf_s(szCommand, _T("set doodad state %u %u"), &dwDoodad, &nState) == 2;
        KGLOG_PROCESS_ERROR(nRetCode);

        nRetCode = SetDoodadState(dwDoodad, nState);
        KGLOG_PROCESS_ERROR(nRetCode);
...
int ExecuteRLCommand(LPCTSTR szCommand, void* pResult)
{
    int nRetCode = false;
    int nResult = false;

    KG_ASSERT_EXIT(szCommand);

    nRetCode = ExecuteTraceCommand(szCommand);
    KG_PROCESS_SUCCESS(nRetCode);

    nRetCode = ExecuteDebugCommand(szCommand);
    KG_PROCESS_SUCCESS(nRetCode);

    nRetCode = ExecuteOptimizationCommand(szCommand);
    KG_PROCESS_SUCCESS(nRetCode);

    nRetCode = ExecuteResourceLimitCommand(szCommand);
    KG_PROCESS_SUCCESS(nRetCode);

    nRetCode = ExecuteTerrainLeanCommand(szCommand);
    KG_PROCESS_SUCCESS(nRetCode);

    nRetCode = ExecuteProfileCommand(szCommand, pResult);
    KG_PROCESS_SUCCESS(nRetCode);

    nRetCode = ExecuteDoodadCommand(szCommand, pResult);
    KG_PROCESS_SUCCESS(nRetCode);
