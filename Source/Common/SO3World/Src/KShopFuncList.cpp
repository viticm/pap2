...
int LuaGetShopItemBuyOtherInfo(Lua_State *L)
{
    ...
    Lua_PushNumber(L, pItemTemplate->dwTabType);
    Lua_PushNumber(L, pItemTemplate->dwIndex);
    Lua_PushNumber(L, pItemTemplate->nRequireAmount);
	Lua_PushNumber(L, pItemTemplate->nRequireAchievementRecord);
	Lua_PushNumber(L, pItemTemplate->nAchievementPoint);
...
int LuaBuyItem(Lua_State *L)
{
    ...
    KGLOG_PROCESS_ERROR(pItem->m_Common.bStack || nCount == 1);
...
int LuaBuySoldListItem(Lua_State *L)
{
    ...
    nCost = pShop->GetPlayerSellPrice(pPlayer, ibSoldList, dwX);
    KGLOG_PROCESS_ERROR(nCost >= 0);

    nCost = nCost * pItem->GetStackNum();
    
    nMoney = pPlayer->m_ItemList.GetMoney();
    KG_PROCESS_ERROR_RET_CODE(nCost <= nMoney, ssrcNotEnoughMoney);
