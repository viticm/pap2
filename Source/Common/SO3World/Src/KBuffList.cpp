...
SKILL_RESULT_CODE KBuffList::AddBuff(KBUFF_LIST_NODE &rBuffNode)
{
    ...
    rBuffNode.nLeftActiveCount = max(1, rBuffNode.nLeftActiveCount); // 保证不把非正的跳数加入BuffList
    ...
    for (KBUFF_LIST::iterator it = m_BuffList.begin(), itEnd = m_BuffList.end(); it != itEnd; ++it)
    {
        ...
        if (pNewBuff->m_dwID == pOldBuff->m_dwID)
        {
            ...
            if (pNewBuff->m_bExclude || rBuffNode.dwSkillSrcID == it->dwSkillSrcID) // 同一个释放者释放的相同Buff
            {
                ...
                if (
                    rBuffNode.dwSkillSrcID == it->dwSkillSrcID &&
                    pNewBuff->m_nIntensity == pOldBuff->m_nIntensity &&
                    pNewBuff->m_nLevel == pOldBuff->m_nLevel
                )
                {
                    rBuffNode.nIndex = it->nIndex;
...
{
    BYTE    byRepresentInPos[brpTotal] = {0};
    ...
    for (KBUFF_LIST::iterator it = m_BuffList.begin(), itEnd = m_BuffList.end(); it != itEnd; ++it)
    {
        ...
        nRepresentPos = pBuff->m_nRepresentPos;

        if (nRepresentPos < brpTrample || nRepresentPos >= brpTotal)
        {
            KGLogPrintf(KGLOG_ERR, "[BUFF] Invalid buff represent pos. BuffID: %lu, Represent Position: %d", pBuff->m_dwID, pBuff->m_nRepresentPos);
            continue;
        }

        if (byRepresentInPos[nRepresentPos] > pBuff->m_dwRepresentID)
            continue;

        byRepresentInPos[nRepresentPos] = (BYTE)pBuff->m_dwRepresentID;
    }
    
    memcpy(m_pOwner->m_byRepresentState, byRepresentInPos, sizeof(m_pOwner->m_byRepresentState));
