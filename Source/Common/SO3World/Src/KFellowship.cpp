...
BOOL KGFellowshipMgr::OnLoadFellowshipData(DWORD dwPlayerID, size_t uDataSize, BYTE byData[])
{
    ...
    if (pPlayer)
    {
        time_t  nOldDay = (pPlayer->m_nLastSaveTime - timezone) / (24 * 3600);
        time_t  nNewDay = (g_pSO3World->m_nCurrentTime - timezone) / (24 * 3600);

        if (nNewDay != nOldDay)
        {
            g_pSO3World->m_FellowshipMgr.AttractionFalloff(dwPlayerID, (int)(nNewDay - nOldDay));
            g_pSO3World->m_FellowshipMgr.RefreshDailyCount(dwPlayerID);
        }
    }
...
BOOL KGFellowshipMgr::OnLoadFellowshipDataV1(DWORD dwPlayerID, size_t uDataSize, BYTE byData[])
{
    ...
    BYTE*                   pbyData         = NULL;
    KPlayer*                pSelf           = NULL;
    const int               cnMaxAttraction = g_pSO3World->m_Settings.m_ConstList.nAttractiveLevel[MAX_FELLOWSHIP_ATTRACTIVE_LEVEL - 1];
    ...
    for (int i = 0; i < pFellowshipDBv1->nFriendCount; i++)
    {
        ...
        pFellowship->m_nAttraction  = min(pFellowshipEntry->nAttraction, cnMaxAttraction);
    ...
    g_pSO3World->m_FellowshipMgr.ProcessFellowshipCountAchievement(pSelf);
...
BOOL KGFellowshipMgr::SaveFellowshipData(DWORD dwPlayerID)
{
    ...
    KGLOG_PROCESS_SUCCESS(it == m_OnlineIDSet.end());
...
int KGFellowshipMgr::AddFellowshipAttraction(DWORD dwPlayerID, DWORD dwAlliedPlayerID, int nAttraction, BOOL bBidirectional /* = true */)
{
    ...
    pFellowshipA->m_nAttraction = min(pFellowshipA->m_nAttraction, cnMaxAttraction);

    if (bBidirectional)
    {
        pFellowshipB->m_nAttraction += nAttraction;
        pFellowshipB->m_nAttraction = min(pFellowshipB->m_nAttraction, cnMaxAttraction);
    }

    nLevelA = AttractionValue2Level(pFellowshipA->m_nAttraction);
    nAchievementID = g_pSO3World->m_Settings.m_ConstList.nAttractiveAchievement[nLevelA - 1];
    if (pPlayerA->m_pScene && nLevelA > 0 && nAchievementID && !pPlayerA->m_Achievement.IsAchieve(nAchievementID))
        pPlayerA->m_Achievement.Acquire(nAchievementID);

    if (bBidirectional)
    {
        nLevelB = AttractionValue2Level(pFellowshipB->m_nAttraction);
        nAchievementID = g_pSO3World->m_Settings.m_ConstList.nAttractiveAchievement[nLevelB - 1];
        if (pPlayerB->m_pScene && nLevelB > 0 && nAchievementID && !pPlayerB->m_Achievement.IsAchieve(nAchievementID))
            pPlayerB->m_Achievement.Acquire(nAchievementID);
    }
...
BOOL KGFellowshipMgr::ProcessFellowshipCountAchievement(KPlayer* pPlayer)
{
    BOOL        bResult     = false;
    int         nCount      = 0;
    const int   cnTableSize = sizeof(g_pSO3World->m_Settings.m_ConstList.nCountAchievement) / sizeof(g_pSO3World->m_Settings.m_ConstList.nCountAchievement[0]);

    KGLOG_PROCESS_ERROR(pPlayer);
    KGLOG_PROCESS_ERROR(pPlayer->m_pScene);

    nCount = CountFellowship(pPlayer->m_dwID);

    for (int i = 0; i < cnTableSize; i++)
    {
        int                 nAchievementID  = g_pSO3World->m_Settings.m_ConstList.nCountAchievement[i];
        int                 nLastCount      = 0;
        int                 nDeltaCount     = 0;
        ...
        pInfo = g_pSO3World->m_Settings.m_AchievementInfoList.GetInfo(nAchievementID);

        if (!pInfo)
            continue;

        if (pPlayer->m_Achievement.IsAchieve(nAchievementID))
            continue;

        nLastCount  = pPlayer->m_Achievement.GetCount(nAchievementID);
        nDeltaCount = nCount - nLastCount;
        nDeltaCount = min(nDeltaCount, pInfo->nTriggerVal - nLastCount);

        pPlayer->m_Achievement.AddCount(nAchievementID, nDeltaCount);
    }
...
{
    const int cnTableSize = sizeof(g_pSO3World->m_Settings.m_ConstList.nAttractiveLevel) / sizeof(g_pSO3World->m_Settings.m_ConstList.nAttractiveLevel[0]);
    ...
    for (int i = 0; i < cnTableSize; i++)
    {
        if (nAttraction <= g_pSO3World->m_Settings.m_ConstList.nAttractiveLevel[i] * nCoefficient)
            return i + 1;
    }
