...
int KGSO3WorldClientInterface::BeginForwardCharacterPosition(KCharacter* pCharacter, KPOSITION_RECORD* pPositionRecord)
{
    ...
    KGLOG_PROCESS_ERROR(pPositionRecord);

    pCharacter->RecordPosition(pPositionRecord);

    g_pSO3World->m_bForwardCharacterPosition = true;
...
int KGSO3WorldClientInterface::EndForwardCharacterPosition(KCharacter* pCharacter, KPOSITION_RECORD* pPositionRecord)
{
    ...
    g_pSO3World->m_bForwardCharacterPosition = false;

    KGLOG_PROCESS_ERROR(pCharacter);
    KGLOG_PROCESS_ERROR(pPositionRecord);

    nRetCode = pCharacter->ReversePosition(pPositionRecord);
...
int KGSO3WorldClientInterface::ForwardCharacterPosition(KCharacter* pCharacter, int nFrame)
{
    ...
    nRetCode = pCharacter->ForwardPosition(nFrame);
    KG_PROCESS_ERROR(nRetCode);
...
void KGSO3WorldClientInterface::MoveViewPoint(int nX, int nY, int nZ)
{
    BOOL     bRetCode = false;
    KPlayer* pPlayer  = NULL;
    ...
    KGLOG_PROCESS_ERROR(pPlayer->m_pViewPointRegion);

    bRetCode = pPlayer->m_pScene->IsPointInSceneSpace(nX, nY, nZ);
    KGLOG_PROCESS_ERROR(bRetCode);

    pRegion = pPlayer->m_pScene->GetRegionByPoint(nX, nY);
    KG_PROCESS_SUCCESS(pRegion == NULL);

    KG_PROCESS_ERROR(pRegion != pPlayer->m_pViewPointRegion);

Exit1:
    g_PlayerClient.DoMoveViewPointRequest(nX, nY);

Exit0:
    return;
}

int KGSO3WorldClientInterface::IsPartyMember(DWORD dwLeader, DWORD dwMemberID, BOOL* pbIsMember)
{
    int nRetCode = false;
    int nResult = false;

    KGLOG_PROCESS_ERROR(pbIsMember);
    KGLOG_PROCESS_ERROR(g_pSO3World);
    KGLOG_PROCESS_ERROR(dwLeader == g_pSO3World->m_dwClientPlayerID);   // TODO:目前只支持本地玩家的查询

    *pbIsMember = g_pSO3World->m_TeamClient.IsPlayerInTeam(dwMemberID);
