...
int KDoodad::LuaGenerateLootListByNpcMode(Lua_State* L)
{
    int         nTopIndex       = Lua_GetTopIndex(L);
    DWORD       dwDropTargetID  = ERROR_ID;
    KPlayer*    pPlayer         = NULL;
    KLootList*  pLootList       = NULL;

    KGLOG_PROCESS_ERROR(nTopIndex == 1);
    
    dwDropTargetID = (DWORD)Lua_ValueToNumber(L, 1);
    
    KGLOG_PROCESS_ERROR(m_eKind == dkNpcDrop); // 此接口只有这种类型的doodad才能使用

    pPlayer = g_pSO3World->m_PlayerSet.GetObj(dwDropTargetID);
    KGLOG_PROCESS_ERROR(pPlayer);

    pLootList = GenerateLootList(pPlayer, false);
    KGLOG_PROCESS_ERROR(pLootList);
    
    pLootList->ProcessNpcDropItems();

    m_pLootList = pLootList;
    pLootList = NULL;

    m_pLootList->m_bLootListChanged = true;
...
int KDoodad::LuaCanDialog(Lua_State* L)
{
	...
	bRetCode = g_InRange(pPlayer, this, COMMON_PLAYER_OPERATION_DISTANCE);
	KG_PROCESS_ERROR(bRetCode);
...
DEFINE_LUA_CLASS_BEGIN(KDoodad)
    ...
    REGISTER_LUA_FUNC(KDoodad, GenerateLootListByNpcMode)
#endif //_SERVER      
