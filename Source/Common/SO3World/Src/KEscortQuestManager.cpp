...
BOOL KEscortQuestManager::AddEscortQuest(DWORD dwQuestID, KPlayer* pPlayerAccept, KNpc* pNpcAccept)
{
    ...
    bRetCode = InvitePlayerInTeam(dwQuestID, pPlayerAccept);
...
BOOL KEscortQuestManager::StartEscortQuest(DWORD dwQuestID)
{
	...
	if (bRetCode && g_pSO3World->m_ScriptCenter.IsFuncExist(pQuestInfo->dwScriptID, SCRIPT_ON_START_QUEST))
	{
        int nTopIndex = 0;
		...
		g_pSO3World->m_ScriptCenter.CallFunction(pQuestInfo->dwScriptID, SCRIPT_ON_START_QUEST, 0);
...
BOOL KEscortQuestManager::EndEscortQuest(DWORD dwQuestID)
{
	...
	bRetCode = g_pSO3World->m_ScriptCenter.IsScriptExist(pQuestInfo->dwScriptID);
	if (bRetCode && g_pSO3World->m_ScriptCenter.IsFuncExist(pQuestInfo->dwScriptID, SCRIPT_ON_END_ESCORT_QUEST))
	{
        int nTopIndex = 0;

		g_pSO3World->m_ScriptCenter.SafeCallBegin(&nTopIndex);
		g_pSO3World->m_ScriptCenter.PushValueToStack(m_pScene);
		g_pSO3World->m_ScriptCenter.CallFunction(pQuestInfo->dwScriptID, SCRIPT_ON_END_ESCORT_QUEST, 0);
		g_pSO3World->m_ScriptCenter.SafeCallEnd(nTopIndex);
	}
...
BOOL KEscortQuestManager::InvitePlayerInTeam(DWORD dwQuestID, KPlayer* pPlayerAccept)
{
    BOOL                         bResult = false;
    BOOL                         bRetCode = false;
    KQuestInfo*                  pQuestInfo = NULL;
