...
#endif
    m_dwSystemTeamID        = INVALID_PARTY_ID;
    m_dwTeamID              = INVALID_PARTY_ID;
...
BOOL KPlayer::Activate(void)
{
	...
	if (m_dwBankNpcID && m_pCell)
	{
		...
		{
			//检查距离,超过了就关闭银行
			bRetCode = g_InRange(this, pNpc, DIALOG_RANGE);
			if (!bRetCode)
			{
				m_dwBankNpcID = ERROR_ID;
    ...
    if ((g_pSO3World->m_nGameLoop - m_dwID) % (GAME_FPS * 128) == 0)
    {
        ScriptActivate();
    }
...
#ifdef _SERVER
    if ((g_pSO3World->m_nGameLoop - m_dwID) % g_pSO3World->m_Settings.m_ConstList.nPartyAttractiveInterval == 0)
    {
    ...
    if ((!m_pScene->m_bSaved) && m_nBanishTime == 0 && (g_pSO3World->m_nGameLoop - (int)m_dwID) % (GAME_FPS * 5) == 0)
    {
        ...
        if (dwSceneOwner != ERROR_ID && m_dwID != dwSceneOwner)
        {
            ...
            else
            {
                ...
                bRetCode = g_pSO3World->m_TeamServer.IsPlayerInParty(m_dwTeamID, dwSceneOwner);
                if (!bRetCode)
                {
                    m_nBanishTime = g_pSO3World->m_nCurrentTime + BANISH_PLAYER_WAIT_SECONDS;
                    g_PlayerServer.DoMessageNotify(
    ...
    {
        DWORD dwSceneOwner  = m_pScene->m_dwOwnerID;
        BOOL  bInOwnerParty = false;

        if (m_dwTeamID != INVALID_PARTY_ID)
        {
            bInOwnerParty = g_pSO3World->m_TeamServer.IsPlayerInParty(m_dwTeamID, dwSceneOwner);
        }

        if (m_dwID == dwSceneOwner || bInOwnerParty)
        {
            m_nBanishTime = 0;
    ...
    if (
        g_pSO3World->m_Settings.m_ConstList.bUsePlayerReviveCtrl && 
        m_ReviveCtrl.nNextCheckReviveFrame == g_pSO3World->m_nGameLoop
    )
        CheckRevive(false);
#endif
...
#ifdef _SERVER
        if (nOpenFrames > 0 && IS_PLAYER(m_dwID))
        {
...
{
    if (m_dwSystemTeamID != INVALID_PARTY_ID)
...
BOOL KPlayer::Save(size_t* puUsedSize, BYTE* pbyBuffer, size_t uBufferSize)
{
    ...
    *puUsedSize = nRoleDataLen;

    snprintf(szFileName, sizeof(szFileName), "savelog/Save_Server%d", m_dwID);
...
BOOL KPlayer::ShareQuest(DWORD dwQuestID)
{
	...
	KGLOG_PROCESS_ERROR(m_dwTeamID != INVALID_PARTY_ID);
...
BOOL KPlayer::IsFormationLeader()
{
    ...
    KG_PROCESS_ERROR(m_dwTeamID != INVALID_PARTY_ID);
...
BOOL KPlayer::SyncFormationCoefficient()
{
    ...
    KG_PROCESS_SUCCESS(m_dwTeamID == INVALID_PARTY_ID); // 没有队伍就不用同步了
...
void KPlayer::ProcessCampPK(DWORD dwKillerID)
{
    ...
    // 给同步范围内的队友加威望
    if (pKiller->m_dwTeamID != INVALID_PARTY_ID && pKiller->m_eCamp != m_eCamp)
    {
        KTeam* pTeam = NULL;
