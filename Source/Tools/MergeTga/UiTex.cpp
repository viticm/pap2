...
int KTgaBlock::CreateTexture()
{
	...
	strncat(szTempPath, "KSG_JX3_MERGE_TGA_TEMP.TGA", MAX_PATH);
	szTempPath[MAX_PATH - 1] = _T('\0');

	nRetCode = ExportTga.Create(GetWidth(), GetHeight());
	KG_PROCESS_ERROR(nRetCode);
	memcpy(ExportTga.GetDataBufferPointer(), GetBufferPtr(), GetBufferSize());
	nRetCode = ExportTga.SaveDataBufferToTgaFile(szTempPath);
	KG_PROCESS_ERROR(nRetCode);

	hr = D3DXGetImageInfoFromFile(szTempPath, &ImageInfo);
	KG_COM_PROCESS_ERROR(hr);

	hr = D3DXCreateTextureFromFileEx(
		KShow::GetSelf().GetDevice(), szTempPath, ImageInfo.Width, ImageInfo.Height,
		1, 0, D3DFMT_A8R8G8B8, D3DPOOL_MANAGED, D3DX_FILTER_NONE, D3DX_FILTER_NONE, 0, NULL, NULL, &m_pTextrue);
	KG_COM_PROCESS_ERROR(hr);

	nResult = true;
Exit0:
	ExportTga.Release();
	if (szTempPath[0] != _T('\0'))
		::DeleteFile(szTempPath);

	return nResult;
}

LPDIRECT3DTEXTURE9 KTgaBlock::GetTexture()
{
	return m_pTextrue;
}
