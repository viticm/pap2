...
int KTgaBlockLoader::LoadTga(LPCTSTR pcszTgaFile)
{
	...
	while (KTgaManager::GetSelf().IsTgaExist(pcszTgaFile))
	{
		TCHAR szRelName[MAX_PATH];
		_tcscpy(szNewName, pcszTgaFile);
		...
		*pszClip = _T('\0');
		...
		pcszTgaFile = szNewName;
		pBlock->SetName(pcszTgaFile);
	}
...
int KTgaBlockLoader::ReadText(LPCTSTR pcszText, int nLoadStyle)
{
	...
	while (1 == fscanf(fpTxt,"%d",&TxtFileData.nFrameNum))
	{
        ...
        fgets(szFile, sizeof(szFile), fpTxt);
		if (szFile[_tcslen(szFile) - 1] == _T('\n'))
			szFile[_tcslen(szFile) - 1] = _T('\0');
		LPCTSTR pcszClip = szFile;
		while(*pcszClip == _T('\t'))
			pcszClip++;
		_tcscpy(TxtFileData.szTxtFile, pcszClip);
