...
void AddOptionValue(const OA_CHAR *name, oaOptionDataType value_type, const oaValue *value)
{
  ...
  {
    case OA_TYPE_STRING:
      fprintf(stderr, "Current option (string)'%s' = %s\n", name, value->String);
      break;

    case OA_TYPE_INT:
      fprintf(stderr, "Current option (int)'%s' = %d\n", name, value->Int);
      break;

    case OA_TYPE_FLOAT:
      fprintf(stderr, "Current option (float)'%s' = %f\n", name, value->Float);
      break;

    case OA_TYPE_ENUM:
      fprintf(stderr, "Current option (enum)'%s' = %s\n", name, value->Enum);
      break;

    case OA_TYPE_BOOL:
      fprintf(stderr, "Current option (bool)'%s' = %d\n", name, value->Bool);
      break;

    default:
      fprintf(stderr, "Current option (unknown type)'%s' = ???", name);
  }
}

void AddBenchmark(const oaChar *benchmark_name)
{
  fprintf(stderr, "IN AddBenchmark: %s\n", benchmark_name);
...
static void EndBenchmark(void)
{
  ...
  fprintf(stderr, "Benchmark ended\n");
  fprintf(stderr, "  Total time = %ds\n", TotalTime);
  fprintf(stderr, "  Avg. FPS = %f\n", AvgFPS);
}
                           ...
                           const oaValue *value)
{
  ...
  {
    case OA_TYPE_STRING:
      fprintf(stderr, "Result value (string)'%s' = %s\n", name, value->String);
      break;

    case OA_TYPE_INT:
      fprintf(stderr, "Result value (int)'%s' = %d\n", name, value->Int);
      break;

    case OA_TYPE_FLOAT:
      fprintf(stderr, "Result value (float)'%s' = %f\n", name, value->Float);
      break;

    case OA_TYPE_ENUM:
      fprintf(stderr, "Result value (enum)'%s' = %s\n", name, value->Enum);
      break;

    case OA_TYPE_BOOL:
      fprintf(stderr, "Result value (bool)'%s' = %d\n", name, value->Bool);
      break;


    default:
      fprintf(stderr, "Result value (unknown type)'%s' = ???", name);
  }
}
                                               ...
                                               oaVersion *version)
{
  ...
  /* The function table must be zeroed out, and the TableSize must be set
     for plugin initialization to work properly when loaded from the app */
