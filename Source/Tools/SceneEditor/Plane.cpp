...
Plane::Plane(float fx, float fy, float fz, float fd, bool bNormalized): m_vNormal(fx, fy, fz), m_fDistanceToOrigin(fd)
{
	if(!bNormalized)
	{
		float fl = ModelSFX::GetVec3Len(m_vNormal);
		if(FEqualZero(fl))
		{
			fl = 1.f;
		}
...
bool Plane::Split(const vector<D3DXVECTOR3>& vIn, vector<D3DXVECTOR3>& vFront, vector<D3DXVECTOR3>& vBack) const
{
	if(vIn.empty())
	{
		return false;
	...
	for(iNextIndex = 0; iNextIndex < (int)NumOfInVectors; ++iNextIndex)
	{
		...
		if((plThis == RL_FRONT && plNext == RL_BACK) || (plThis == RL_BACK && plNext == RL_FRONT))
		{
			D3DXVECTOR3 vSplit = SplitLine(vIn[iThisIndex], vIn[iNextIndex]);
