...
CTransformOutputPin::SetMediaType(const CMediaType* pmtOut)
{
	...
	DbgLog((LOG_ERROR1,0,TEXT("*** This filter is accepting an output media type")));
	DbgLog((LOG_ERROR1,0,TEXT("    that it can't currently transform to.  I hope")));
	DbgLog((LOG_ERROR1,0,TEXT("    it's smart enough to reconnect its input.")));
    }
#endif
