...
DWORD CSourceStream::ThreadProc(void) {
	...
	com = GetRequest();
	if (com != CMD_INIT) {
	    DbgLog((LOG_ERROR1, 1, TEXT("Thread expected init command")));
    ...
    if (FAILED(hr)) {
        DbgLog((LOG_ERROR1, 1, TEXT("CSourceStream::OnThreadCreate failed. Aborting thread.")));
        OnThreadDestroy();
	...
	case CMD_RUN:
	    DbgLog((LOG_ERROR1, 1, TEXT("CMD_RUN received before a CMD_PAUSE???")));
	    // !!! fall through???
	...
	default:
	    DbgLog((LOG_ERROR1, 1, TEXT("Unknown command %d received!"), cmd));
    ...
    if (FAILED(hr)) {
        DbgLog((LOG_ERROR1, 1, TEXT("CSourceStream::OnThreadDestroy failed. Exiting thread.")));
        return 1;
    }
...
HRESULT CSourceStream::DoBufferProcessingLoop(void) {
	...
	while (!CheckRequest(&com)) {
		...
		return S_OK;
                ...
                pSample->Release();
		DbgLog((LOG_ERROR1, 1, TEXT("Error %08lX from FillBuffer!!!"), hr));
                DeliverEndOfStream();
	...
	if (com == CMD_RUN || com == CMD_PAUSE) {
	    ...
	    Reply((DWORD) E_UNEXPECTED);
	    DbgLog((LOG_ERROR1, 1, TEXT("Unexpected command!!!")));
	}
    } while (com != CMD_STOP);
