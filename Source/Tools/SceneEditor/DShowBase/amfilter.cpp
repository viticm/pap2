                ...
                ALLOCATOR_PROPERTIES* pActual)
{
    ...
    if (pRequest->cbAlign != 1) {
        DbgLog((LOG_ERROR1, 2, TEXT("Alignment requested was 0x%x, not 1"),
               pRequest->cbAlign));
                ...
                ALLOCATOR_PROPERTIES* pActual)
{
    ...
    if ((-pRequest->cbAlign & pRequest->cbAlign) != pRequest->cbAlign) {
        DbgLog((LOG_ERROR1, 1, TEXT("Alignment requested 0x%x not a power of 2!"),
               pRequest->cbAlign));
    }
    /*  Check the alignment requested */
    if (pRequest->cbAlign == 0 ||
    (SysInfo.dwAllocationGranularity & (pRequest->cbAlign - 1)) != 0) {
        DbgLog((LOG_ERROR1, 1, TEXT("Invalid alignment 0x%x requested - granularity = 0x%x"),
