...
/* For every module and executable we store a debugging level for each of
   the five categories (eg LOG_ERROR1 and LOG_TIMING). This makes it easy
    ...
    for (lKeyPos = 0;lKeyPos < iMAXLEVELS;lKeyPos++) {
        ...
        if (lReturn != ERROR_SUCCESS || dwKeyType != REG_DWORD)  {
            ...
            if (lReturn != ERROR_SUCCESS) {
                DbgLog((LOG_ERROR1,0,TEXT("Could not create subkey %s"),pKeyNames[lKeyPos]));
                dwKeyValue = 0;
            }
        }
        if(fTakeMax)
...
void WINAPI DbgInitGlobalSettings(bool fTakeMax)
{
    ...
    if (lReturn != ERROR_SUCCESS) {
        DbgLog((LOG_ERROR1,0,TEXT("Could not access GLOBAL module key")));
        return;
    }
...
void WINAPI DbgInitModuleSettings(bool fTakeMax)
{
    ...
    if (lReturn != ERROR_SUCCESS) {
        DbgLog((LOG_ERROR1,0,TEXT("Could not access module key")));
        return;
    }
...
{
    DbgLog((LOG_ERROR1,0,TEXT("Assertion FAILED (%hs) at line %d in file %hs"),
           pCondition, iLine, pFileName));
    DebugBreak();
}
...
{
    DbgLog((LOG_ERROR1,0,TEXT("Assertion FAILED (%s) at line %d in file %s"),
           pCondition, iLine, pFileName));
    DebugBreak();
}
