...
BOOL KLogicObstacleGenerator::GenerateSceneLogicObstacleData(IEKG3DScene* pi3DScene, KScene* pLogicScene,BOOL bOnlyAppointCell,	BOOL bClearCell)
{
	...
	m_pProgressBar = new KDlgLogicSaveProgress(AfxGetMainWnd());
    KGLOG_PROCESS_ERROR(m_pProgressBar);

    m_pProgressBar->m_progressModel.SetRange32(0, 100);
    m_pProgressBar->ShowWindow(SW_NORMAL);
    ...
    for (int nY = nY_LogicRegionStart; nY < nY_LogicRegionEnd; nY++)
    {
        for (int nX = nX_LogicRegionStart; nX < nX_LogicRegionEnd; nX++)
        {
            ...
            m_pProgressBar->m_staticModel.SetWindowText(szMessage);
	        m_pProgressBar->m_progressModel.SetPos(nCompleteRegion * 100 / nTotalRegion);
        }
    }

    m_pProgressBar->m_staticModel.SetWindowText("正在进行障碍预计算 ... ...");
    m_pProgressBar->m_progressModel.SetPos(100);
...

	{
		...
		MessageBox(AfxGetMainWnd()->m_hWnd, szMessage, "", MB_OK);
