...
//在KSceneEditorView::OnInitialUpdate有被调用
...
ReLoop:
		...
		while (m_pKeyMapArray[nMapIndex].wActionID)
		{
			...
			//if (pMsg->message == KeyMap[nMapIndex].TriggerMsgID || 0 == KeyMap[nMapIndex].TriggerMsgID)
			{
				if (*pdwKeyState == *(WORD*)&(KeyMap.keystate)) //找到符合的组合键
				{
					...
					//m_pSceneEditor->PreExecAction(KeyMap.wActionID, 1, pMsg->wParam, lParam);//没有用到
				...
				*pdwKeyState != *(WORD*)&(KeyMap.keystate)) //按键状态发生了变化，需要发出通知
			{
				...
				//m_pSceneEditor->PreExecAction(KeyMap.wActionID, 0, pMsg->wParam, lParam);//没有用到
		...
		if (pMsg->message >= WM_MOUSEFIRST && pMsg->message <= WM_MOUSELAST)
		{
			...
			//m_pSceneEditor->SyncPickRay();//同步PickRay是内部加速操作，不应该暴露在接口中，这个操作由响应EXEACTION_MOUSE_MOVE而触发

			SHORT IsLButtomDown = ::GetAsyncKeyState(VK_LBUTTON) & 0x800;
			m_pSceneEditor->ExecAction(EXEACTION_MOUSE_MOVE, IsLButtomDown, pMsg->wParam, pMsg->lParam);
		}

		if (pMsg->message == WM_MOUSEWHEEL)
		{
			SHORT IsMButtomDown = ::GetAsyncKeyState(VK_MBUTTON) & 0x800;
			m_pSceneEditor->ExecAction(EXEACTION_MOUSE_WHEEL, IsMButtomDown, pMsg->wParam, pMsg->lParam);
		}
	}
	return nRet;
}

BOOL CInputProxy::IsCaptured()
{
	return m_bInCapture;
}
