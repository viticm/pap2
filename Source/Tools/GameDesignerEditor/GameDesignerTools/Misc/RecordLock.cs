...
namespace GameDesingerTools
{
    ...
    public class RecordLock
    {
        ...
        private bool InitDBParams(object[] KeyValues)
        {
            ...
            // 尝试解决加解锁时dataAdapter执行sql命令异常的问题 modify by suntao
        ...
        public bool IsMyLock(object[] KeyValues, ref string lockhostname)
        {
            if (KeyValues == null)
                return false;
            ...
            try
            {
                ...
                {
                    string hostname = Helper.GetHostNameByIp(strlockip);
        ...
        private bool UpdateRow(DataTable tbl, DataRow row, int nLastVer, DateTime dtLock, DateTime dtUnLock, int nModId, bool bLock)
        {
            ...
            catch (Exception ex)
            {
                if (val == 0)
                {
                    ...
                    return Helper.SqlUpdateRecord("sys_lock_record", keynames, keys, fields, values, m_conn);
                }
                else
                {
                    tbl.RejectChanges();
    ...
    public class RecordLockEx : RecordLock
    {
        ...
        public RecordLockEx(SqlConnection conn, string strTableName, object[] DBPrimaryKeys, int nModID) : base(conn, strTableName, DBPrimaryKeys, nModID)
        {            
        }
