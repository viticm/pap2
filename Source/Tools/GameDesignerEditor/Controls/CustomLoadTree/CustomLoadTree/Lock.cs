    ...
    class TableLock
    {
        ...
        public bool Lock()
        {
            ...
            try
            {
                ...
                string strlockip = row["lockip"] == null ? "" : row["lockip"].ToString().Trim();
                ...
                {
                    string strlockhost = row["lockhostname"] == null ? "" : row["lockhostname"].ToString().Trim();
                    string warning = string.Format("任务树目前被 {0} [{1}] 锁定，让其重开编辑器即可解锁。", strlockhost, strlockip);
