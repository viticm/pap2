                ...
                if (line != "") // 跳过空的数据行
                {
                    ...
                    if (firstLine)
                    {
                        ...
                        for (int i = 0; i < data.Length; i++)
                        {
                            ...
                            if (data[i].ToLower().Contains("id"))
                                dataColumn = new DataColumn(data[i], Type.GetType("System.Int32"));
                            else
                                dataColumn = new DataColumn(data[i]);
                    ...
                    else
                    {
                        ...
                        for (int i = 0; i < data.Length; i++)
                        {
                            if (table.Columns[i].ColumnName.ToLower().Contains("id"))
                            {
                                if (data[i] == string.Empty)
                                    dataRow[i] = DBNull.Value;
                                else
                                    dataRow[i] = Convert.ToInt32(data[i]);
                            }
                            else
                            {
                                dataRow[i] = data[i];
