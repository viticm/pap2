...
namespace OfficialWebDB
{
    class Helper
    {
        ...
        public static void AddLog(string err)
        {
            StreamWriter writer = new StreamWriter("./WebDBlog.txt", true, Encoding.GetEncoding("gb2312"));
            writer.Write(err + "\r\n");
            writer.Close();
        }
        ...
        public static bool ClearDbTable(string tblName, SqlConnection Conn)
        {
            Conn.Open();
            try
            {
        ...
        public static bool ExecSqlCmd(string sql, SqlConnection Conn)
        {
            ...
            {
                string err = string.Format("在执行 {0} 时发生错误。\r\n\r\n错误信息：{1}", sql, ex.Message);
                MessageBox.Show(err);
                return false;
            }
            Conn.Close();
