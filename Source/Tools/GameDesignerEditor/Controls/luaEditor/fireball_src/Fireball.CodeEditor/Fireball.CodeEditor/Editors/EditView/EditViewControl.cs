...
namespace Fireball.Windows.Forms.CodeEditor
{
    ...
    {
        private int ignoreCharSignal = 0; // 需要忽略WM_CHAR消息的次数

        private string mFindText = string.Empty;

        #region General Declarations

        private int debugRowIndex = -1; // 当前debug的行号
        private int lastDebugRowIndex = -1; // 上一次debug行的位置 
        private Color lastDebugRowBackColor; // 上一次debug行的颜色
        ...
        public int DebugRowIndex
        {
            set
            {
                ...
                if ((debugRowIndex == -1 && value >=0)|| (debugRowIndex != lastDebugRowIndex))
                {
                    if (lastDebugRowIndex != -1)
                    {
                        Document.VisibleRows[lastDebugRowIndex].BackColor = lastDebugRowBackColor;
                    }

                    lastDebugRowBackColor = Document.VisibleRows[debugRowIndex].BackColor;
                    Document.VisibleRows[debugRowIndex].BackColor = Color.Yellow;
                    lastDebugRowIndex = debugRowIndex;
                }
            }

            get
            {
                return debugRowIndex;
            }
        }
        ...
        {
            if (this._CodeEditor != null && !this._CodeEditor.DisableAutoList && (this._AutoList == null || this._AutoList.IsDisposed))
            {
        ...
        {
            if (this._CodeEditor != null && !this._CodeEditor.DisableInfoTip && (this._InfoTip == null || this._InfoTip.IsDisposed))
            {
        ...
        {
            SelectPattern(RowIndex, Column, Length);
        }
                ...
                else
                {
                    if (x > View.TextMargin - 8)
                    {
                        ...
                        else
                        {
                            ...
                            if (w != null) // 鼠标放在单词上就可以触发事件了，不必检查单词的样式
                            {
                                WordMouseEventArgs e = new WordMouseEventArgs();
                                e.Pattern = w.Pattern;
                                e.Button = MouseButtons.None;
        ...
        {
            if (this.InfoTip == null || this.InfoTip.IsDisposed || this.AutoList == null || this.AutoList.IsDisposed)
                return;
        ...
        public bool SelectNext(string Pattern, bool MatchCase, bool WholeWords, bool UseRegEx)
        {
            ...
            for (int i = this.Caret.Position.Y; i < this.Document.Count; i++)
            {
                ...
                else
                {
                    ...
                    // 修复查找与替换不会处理一行多次匹配的bug。
                    string tToExamin = t;
                    Col = tToExamin.IndexOf(pattern);
                    int iCutSubStrAdjust = 0; // 一行多次查找，需要砍掉上次匹配的部分（substr）。记录砍掉的长度
                    while (Col != -1)
                    {
                        if ((Col >= StartCol) || (i > StartRow && Col >= 0))
                        {
                        ...
                        {
                            if (Col + pattern.Length >= tToExamin.Length + iCutSubStrAdjust) // 不会在此行再找到了
                            {
                                break;
                            }
                            else
                            {
                                int thisTimeCut = Col + pattern.Length - iCutSubStrAdjust;
                                tToExamin = tToExamin.Substring(thisTimeCut);
                                iCutSubStrAdjust += thisTimeCut;
        ...
        public bool SetupFindText()
        {
            int StartCol = this.Caret.Position.X;
            int StartRow = this.Caret.Position.Y;
            string pattern = this.Selection.Text;
            if (pattern.Contains("\r\n"))
            {
                return false;
            }
            ...
            {
                string tmp = ".,+-*^\\/()[]{}@:;'??#%& \t=<>";
                string text = Document[StartRow].Text;
                int i = 0;
                for (i = StartCol; i >= 0; --i)
                {
                    if (tmp.IndexOf(text[i]) >= 0)
                    {
                        break;
                    }
                }
        ...
        public bool FindNextSelectText()
        {

            string pattern = mFindText;
            int StartCol = this.Caret.Position.X;
            int StartRow = this.Caret.Position.Y;
            ...
            for (int index = StartRow; index < Document.Count; ++index)
            {

                string text;
                if (index == StartRow)
                {
                    text = this.Document[index].Text.Substring(StartCol);
                }
                else

                {
                    text = this.Document[index].Text;
                }
                ...
                if (col != -1)
                {
                    ...
                    SelectPattern(index, col, pattern.Length);
                    return true;
                }
            }
            return false;
        ...
        public bool FindPreviousSelectText()
        {
            ...
            int StartCol = this.Caret.Position.X - pattern.Length;
            int StartRow = this.Caret.Position.Y;


            for (int index = StartRow; index >= 0; --index)
            {

                string text;
                if (index == StartRow)
                {
                    text = this.Document[index].Text.Substring(0, StartCol);
                }
                else
                {
                    text = this.Document[index].Text;
                }
                int col = text.LastIndexOf(pattern);
        ...
        public bool InfoTipVisible
        {
            ...
            {
                CreateInfoTip();
                if (this.InfoTip == null || this.InfoTip.IsDisposed)
                    return;
        ...
        protected override void OnKeyDown(KeyEventArgs e)
        {
            ...
            if (!e.Handled && AutoListVisible)
            {
                ...
                if(e.Alt == true && (e.KeyCode== Keys.Left || e.KeyCode == Keys.Right))
                {
                    AutoList.SendKey((int)e.KeyCode);
                    e.Handled = true;
                    return;
