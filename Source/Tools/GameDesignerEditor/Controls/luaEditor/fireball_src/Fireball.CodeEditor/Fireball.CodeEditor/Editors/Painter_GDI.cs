...
namespace Fireball.Windows.Forms.CodeEditor.Painter
{
    ...
    public class Painter_GDI : IPainter
    {
        ...
        private void RenderAll2(Graphics g)
        {
            ...
            // 再根据InfoTip的显示否，决定AutoList的位置怎么摆
            if (Control.AutoListStartPos != null)
            {
                if (Control.AutoListVisible)
                {
                    ...
                    else
                    {
                        ...
                        // 让autolist显示到上面去，不要挡住InfoTip
                        if (Control.InfoTip.Visible == true)
                        {
                            Control.AutoList.Left = alP.X;
                            Control.AutoList.Top = Control.InfoTip.Top - Control.AutoList.Height - 24;
                            Control.AutoList.Tag = "top";
                            Control.AutoList.ToolBoxDock = DockStyle.Top;
                        }
                        else
                        {
                            Control.AutoList.Location = alP;
                            Control.AutoList.Tag = "";
                            Control.AutoList.ToolBoxDock = DockStyle.Bottom;
                        }
        ...
        private void RenderRow(Graphics g, int RowIndex, int RowPos)
        {
            ...
            if (RowIndex < this.Control.Document.Count && RowIndex >= 0)
            {
                ...
                // 调试行的背景色换成黄色
                if (RowIndex == Control.DebugRowIndex)
                {
