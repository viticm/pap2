...
namespace GameLuaEditor
{
    public static class Helper
    {
        ...
        private static StringBuilder logText = new StringBuilder(); // 日志文件

        // 日志等级
        public enum LogLevel { Trace, Warning, Error };
    ...
    #region InputBox
        ...
        public static void RecordLog(string message, LogLevel logLevel)
        {
            ...
            logText.AppendLine(string.Format("{0} [{1}]: {2}", 
                                             DateTime.Now, levelDescription, message));
        }
        ...
        public static void SaveLog()
        {
            if (logText.Length > 0)
            {                
                string fileName = Path.Combine(Application.StartupPath, @"logs\LuaEditor");
                fileName = Path.Combine(fileName, string.Format("{0}.txt", DateTime.Now.ToString().Replace(" ", "-").Replace(":", "-")));
                StringToFile(logText.ToString(), fileName);
            }
        }
