...
int KTgaManager::CalculateBlockPos(int nIndex, POINT &ptWanted)
{
    ...
    for (i = 0; i < nCount; ++i) 
    {
        ...
        {
            ptWanted.x = rcPosed.left - 1 - (rcSlef.right - rcSlef.left);
        ...
        {
            ptWanted.y = rcPosed.top - 1 - (rcSlef.bottom - rcSlef.top);
    ...
    for (int nIndex = 0; nIndex < nCount; ++nIndex)
    {
        ...
        for (int i = 0; i <= nDesiredHeight - nSelfHeight; ++i)
        {
            ...
            for (int nIndexSeted = 0; nIndexSeted < nCount; ++nIndexSeted)
            {
                ...
                if (rcPosed.right + nSelfWidth > nDesiredWidth)
                {
                    pBlockSelf->SetLeft(0);
                    break;
                }

                pBlockSelf->SetLeft(g_GetCorrectPosition(rcPosed.right + 1));
                nIndexSeted = -1;
            }
    ...
    for (int nIndex = 0; nIndex < nCount; ++nIndex)
    {
        ...
        for (int i = nDesiredWidth - nSelfWidth; i >= 0; --i)
        {
            ...
            for (int nIndexSeted = 0; nIndexSeted < nCount; ++nIndexSeted)
            {
                ...
                if (rcPosed.bottom + nSelfHeight > nDesiredHeight)
                {
                    pBlockSelf->SetTop(0);
                    break;
                }

                pBlockSelf->SetTop(g_GetCorrectPosition(rcPosed.bottom + 1));
                nIndexSeted = -1;
            }
