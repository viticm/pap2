// AtlOld3DEngine.cpp : Implementation of CAtlOld3DEngine

#include "stdafx.h"
#include "AtlOld3DEngine.h"
#include "KG3DInterface.h"
#include "./AtlTest3DEngine/SceneEditorCommon.h"
...
#include "KSO3World.h"
#include "./AtlTest3DEngine/Functions.h"
...
//TCHAR* CAtlOld3DEngine::m_strSockets[NUM_SOCKET] = { "s_hat", "s_spine2", "s_spine", "s_rp", "s_lp", "s_rc", "s_lc", "s_rh", "s_lh" };
...
extern BOOL g_bProgressInited;
BOOL g_bEngineEnable = TRUE;

STDMETHODIMP CAtlOld3DEngine::ShowProgressForm(BOOL bShow)
{
	if (bShow)
	{
...
STDMETHODIMP CAtlOld3DEngine::SaveLogicScene(BSTR iniFile, LONG ptr3DScene) // iniFile 是 *.Map.Logical 之类的文件
{
...
STDMETHODIMP CAtlOld3DEngine::Init(LONG hwndBase, BSTR strEnginePath, BSTR strStartUpPath)
{
	USES_CONVERSION;
...
STDMETHODIMP CAtlOld3DEngine::GetInitInfo(LONG* hwndBase, BSTR* strEnginePath, BSTR* strStartUpPath)
{
	USES_CONVERSION;
	*hwndBase = (LONG)m_hRenderWndBase;
	*strEnginePath = m_strEnginePath;
	*strStartUpPath = m_strStartUpPath;
	return S_OK;
}

STDMETHODIMP CAtlOld3DEngine::FileToImage(BSTR src3DFileName, BSTR desImageFileName) // 文件名不带后缀，如果是动作的也不带001，002之类。
{
	USES_CONVERSION;	
	HRESULT hr = E_FAIL;
...
STDMETHODIMP CAtlOld3DEngine::SaveMdlWithAniToImages(LONG ptrModel, BSTR fileName)
{
	USES_CONVERSION;
...
STDMETHODIMP CAtlOld3DEngine::SaveModelToImageFile(BSTR fileName)
{
	// 	USES_CONVERSION;
	HRESULT hr = E_FAIL;
	// 
...
STDMETHODIMP CAtlOld3DEngine::ActivateScene(BSTR editor_name)
{
...
STDMETHODIMP CAtlOld3DEngine::GetCurSceneIndex(LONG* index)
{
	*index = m_index;
	return S_OK;
}

STDMETHODIMP CAtlOld3DEngine::ActivateSceneByIndex(LONG index)
{
	m_index = index;
	if (index >= 0 && index < (int)m_p3DScenesVec.size())
		m_p3DScenesVec[index]->SetCurrentOutputWindow(m_OutputWindowIDs[index]);
...
STDMETHODIMP CAtlOld3DEngine::GetEditorSceneCount(BSTR editor_name, LONG* count)
{
	std::string strEditorName(BSTR_TO_STRING(editor_name));
	std::map<std::string, INT>::iterator Iter = m_NameHash.find(strEditorName);
	if (Iter == m_NameHash.end())
...
STDMETHODIMP CAtlOld3DEngine::RemoveScene(BSTR editor_name)
{
	std::string strEditorName(BSTR_TO_STRING(editor_name));
	std::map<std::string, INT>::iterator Iter = m_NameHash.find(strEditorName);
	if (Iter == m_NameHash.end())
...
STDMETHODIMP CAtlOld3DEngine::AdjustCameraOrthogonal(LONG iOffset)
{
...
STDMETHODIMP CAtlOld3DEngine::SetCameraInfo(LONG pX, LONG pY, LONG pZ, LONG lX, LONG lY, LONG lZ, LONG width, LONG height)
{
	IEKG3DSceneOutputWnd *piSceneCurOutputWnd = NULL;
	m_p3DScenesVec[m_index]->GetCurOutputWnd(&piSceneCurOutputWnd);
	if (piSceneCurOutputWnd)
...
STDMETHODIMP CAtlOld3DEngine::GetCameraInfo(LONG* pX, LONG* pY, LONG* pZ, LONG* lX, LONG* lY, LONG* lZ, LONG* width, LONG* height)
{
	IEKG3DSceneOutputWnd *piSceneCurOutputWnd = NULL;
	m_p3DScenesVec[m_index]->GetCurOutputWnd(&piSceneCurOutputWnd);
	if (piSceneCurOutputWnd)
...
STDMETHODIMP CAtlOld3DEngine::LoadOneMap_NoOutputWindow(BSTR filename, LONG* ptrScene)
{
...
STDMETHODIMP CAtlOld3DEngine::GetHeight(LONG ptrScene, FLOAT x, FLOAT z, FLOAT* y)
{
...
STDMETHODIMP CAtlOld3DEngine::ReadMapTerrainInfo(LONG ptrScene, LONG* width, LONG* height)
{
	IEKG3DScene* pScene = (IEKG3DScene*)ptrScene;
...
STDMETHODIMP CAtlOld3DEngine::GetEditorSceneSize(LONG ptrScene, LONG* width, LONG* height)
{
	IEKG3DScene* pScene = (IEKG3DScene*)ptrScene;
...
STDMETHODIMP CAtlOld3DEngine::AddScene(LONG hwndOutput, BSTR editor_name, LONG iOption)
{
...
STDMETHODIMP CAtlOld3DEngine::ToggleRotateModelEditor(void)
{
...
STDMETHODIMP CAtlOld3DEngine::UnInit(void)
{
	//AFX_MANAGE_STATE(AfxGetStaticModuleState());
...
STDMETHODIMP CAtlOld3DEngine::Render(void)
{
	//AFX_MANAGE_STATE(AfxGetStaticModuleState());
...
STDMETHODIMP CAtlOld3DEngine::FrameMove(void)
{
	//AFX_MANAGE_STATE(AfxGetStaticModuleState());
	if (g_pEngineManager && g_bEngineEnable)
...
STDMETHODIMP CAtlOld3DEngine::ToggleEngine(void)
{
	g_bEngineEnable = !g_bEngineEnable;
	return S_OK;
}

STDMETHODIMP CAtlOld3DEngine::OnResizeWindow(void)
{
	//AFX_MANAGE_STATE(AfxGetStaticModuleState());
	if (m_p3DScenesVec[m_index])
...
STDMETHODIMP CAtlOld3DEngine::get_PlayMode(VARIANT_BOOL* pVal)
{
	//AFX_MANAGE_STATE(AfxGetStaticModuleState());
...
STDMETHODIMP CAtlOld3DEngine::put_PlayMode(VARIANT_BOOL newVal)
{
	//AFX_MANAGE_STATE(AfxGetStaticModuleState());
...
STDMETHODIMP CAtlOld3DEngine::OnWndMsg(BSTR editor_name, PMSG WndMsg, INT* pnRet)
{
	//AFX_MANAGE_STATE(AfxGetStaticModuleState());
	...
	INT index = Iter->second;
	if (index < m_InputProxies.size())
		*pnRet = m_InputProxies[index]->CheckInputEvent(pMsg);

	return S_OK;
}

STDMETHODIMP CAtlOld3DEngine::LoadFile(BSTR strFileName)
{
	USES_CONVERSION;
...
STDMETHODIMP CAtlOld3DEngine::LoadAni(BSTR strFileName, BOOL bLoop)
{
	USES_CONVERSION;
...
STDMETHODIMP CAtlOld3DEngine::UnloadFile(BSTR strFileName)
{
	//AFX_MANAGE_STATE(AfxGetStaticModuleState());
...
STDMETHODIMP CAtlOld3DEngine::UnloadPlayerModel(void)
{
	// todo:
	return S_OK;
}

STDMETHODIMP CAtlOld3DEngine::SetAllPluginModelScale(FLOAT fScale)
{
...
STDMETHODIMP CAtlOld3DEngine::LoadPlayerModel(BSTR strFileName)
{
	//LONG count = -1;
...
STDMETHODIMP CAtlOld3DEngine::SetModelScale(FLOAT fScale)
{
	IKG3DModel *pModel = NULL;
	m_RepresentObjects[m_index]->GetModel(pModel, 0);
	D3DXVECTOR3 vScale(fScale, fScale, fScale);
...
STDMETHODIMP CAtlOld3DEngine::LoadNPCMarkSfxFile(BSTR sfx_name, FLOAT fHeight)
{
...
STDMETHODIMP CAtlOld3DEngine::LoadSelectorSfxFile(BSTR sfx_name)
{
	if (FAILED(g_pEngineManager->NewKG3DRepresentObject(REPRESENTOBJECT_DEFAULT, 0, 0, &m_RepresentObjectSelectors[m_index])))
		return E_FAIL;
...
STDMETHODIMP CAtlOld3DEngine::ScaleSelectorSfx(FLOAT x, FLOAT y, FLOAT z)
{
	IKG3DModel* pModel = NULL;
	m_RepresentObjects[m_index]->GetModel(pModel, 1);
	if (pModel)
...
/* 0 Left 1 Right */
STDMETHODIMP CAtlOld3DEngine::LoadSkillSfx(BSTR strFileName, FLOAT scale, BSTR socketName, LONG iIndex)
{
...
STDMETHODIMP CAtlOld3DEngine::LoadPlaneForSelector(BSTR strFileName)
{
	if (FAILED(g_pEngineManager->NewKG3DRepresentObject(REPRESENTOBJECT_DEFAULT, 0, 0, &m_RepresentObjectSelectors[m_index])))
		return E_FAIL;
...
STDMETHODIMP CAtlOld3DEngine::LoadBillboardSfx(BSTR strFileName)
{
	if (FAILED(g_pEngineManager->NewKG3DRepresentObject(REPRESENTOBJECT_DEFAULT, 0, 0, &m_RepresentObjectSelectors[m_index])))
		return E_FAIL;
...
STDMETHODIMP CAtlOld3DEngine::SetBillboardHeight(INT npc_height, INT head_top_adjust)
{
	IKG3DModel* pModel = NULL;
	m_RepresentObjects[m_index]->GetModel(pModel, 2);
	if (pModel)
...
STDMETHODIMP CAtlOld3DEngine::SetCaptionHeight(FLOAT fHeight)
{
	IKG3DModel* pModel = NULL;
	m_RepresentObjects[m_index]->GetModel(pModel, 0);
	if (pModel)
...
STDMETHODIMP CAtlOld3DEngine::SetCaptionText(BSTR strName, BSTR strTitle)
{
	IKG3DModel* pModel = NULL;
	m_RepresentObjects[m_index]->GetModel(pModel, 0);
	if (pModel)
...
STDMETHODIMP CAtlOld3DEngine::BindToPlayer(BSTR strFileName, DWORD dwType, BSTR strBindToObj)
{
...
STDMETHODIMP CAtlOld3DEngine::ZoomView(LONG lDelta)
{
#ifdef USE_SCENE_EDITOR
	GETEDITOR() E_FAIL; GETSCENEBASE() E_FAIL;
#else
...
// STDMETHODIMP CAtlOld3DEngine::RotateModel(INT xoff, INT yoff)
// {
// 	IKG3DModel* pModel = NULL;
// 	m_RepresentObjects[m_index]->GetModel(pModel, 0);
// 	D3DXQUATERNION qtrn;
...
STDMETHODIMP CAtlOld3DEngine::ChangeModelPart(BSTR filename, INT PartID)
{
	HRESULT hr = E_FAIL;
	IKG3DModel* pModel = NULL;
	IKG3DModel* pPlayerModel = NULL;
...
STDMETHODIMP CAtlOld3DEngine::LoadFace(BSTR strfilename)
{
	HRESULT hr = E_FAIL;
	SAFE_RELEASE(m_pSelectedObj);
	IEKG3DModelTable* piModelTable = NULL;
...
STDMETHODIMP CAtlOld3DEngine::ItemsClear()
{
...
STDMETHODIMP CAtlOld3DEngine::SaveMDLToFile( BSTR strFileName)
{
	HRESULT hr = E_FAIL;
...
STDMETHODIMP CAtlOld3DEngine::LoadMaterial(BSTR strFileName)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
	HRESULT hr = E_FAIL;
	IKG3DModel *pModel = NULL;
...
STDMETHODIMP CAtlOld3DEngine::LoadAnimation(BSTR strFileName, BOOL bLoopPlay)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
	USES_CONVERSION;
...
STDMETHODIMP CAtlOld3DEngine::LoadPluginModel(BSTR strFileName, BSTR strBindDest)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
	HRESULT hr = E_FAIL;
	IKG3DModel* pPlayerModel = NULL;
...
STDMETHODIMP CAtlOld3DEngine::LoadPluginMaterial(BSTR strSocketName, BSTR strMtlFile)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
...
STDMETHODIMP CAtlOld3DEngine::ClearPluginModel(BSTR strBindDest)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
...
STDMETHODIMP CAtlOld3DEngine::GetFileNameHash(BSTR fileName, DWORD* hashid)
{
	*hashid = g_FileNameHash(BSTR_TO_STRING(fileName));
	return S_OK;
}

STDMETHODIMP CAtlOld3DEngine::QuaternionRotationByNDir(FLOAT* x, FLOAT* y, FLOAT* z, FLOAT* w, LONG nDir)
{
...
STDMETHODIMP CAtlOld3DEngine::GetAniFrameCount(BSTR strAniFileName, LONG* count)
{
	...
	{
		MessageBox(NULL, TEXT("CAtlOld3DEngine::GetAniFrameCount参数错误，文件后缀名不符。"), NULL, 0);
	}

	return S_OK;
}

STDMETHODIMP CAtlOld3DEngine::GetBBoxXY(BSTR mdl, FLOAT* x, FLOAT* y)
{
...
STDMETHODIMP CAtlOld3DEngine::GetMdlBBoxMax2DLength(BSTR mdl, FLOAT* len)
{
	*len = 0.0f;
