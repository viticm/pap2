...
BOOL KAuctionClient::UpdateItemList(int nIndex, KAUCTION_PAGE_HEAD* pPage)
{
    ...
    BYTE*               pbyOffset   = pPage->bySaleList;
    size_t              uLeftSize   = pPage->uSize;
    ...
    while (uLeftSize > 0)
    {
        ...
        KGLOG_PROCESS_ERROR(uLeftSize >= sizeof(KAUCTION_PAGE_NODE) + pNode->uItemSize);
        ...
        pbyOffset += sizeof(KAUCTION_PAGE_NODE) + pNode->uItemSize;
        uLeftSize -= sizeof(KAUCTION_PAGE_NODE) + pNode->uItemSize;
    }
