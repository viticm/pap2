...
BOOL KPlayerServer::Detach(int nConnIndex)
{
	...
	case gsSaveDataForTransmission:
    case gsTransmission:
...
BOOL KPlayerServer::PartyBroadcast(DWORD dwTeamID, void* pvData, size_t uSize)
{
    BOOL           bRetCode = false;
    KTeamBroadcast Func;
...
BOOL KPlayerServer::PartyBroadcastTalk(KTeam* pTeam, DWORD dwTalkerID, void* pvData, size_t uSize)
{
    KPlayer* pPlayer = NULL;
...
BOOL KBroadcastBeHitFunc::operator()(KPlayer* pPlayer)
{
    ...
    if (m_pExceptSrc)
    {
        ...
        KG_PROCESS_SUCCESS(
            pPlayer->m_dwTeamID != ERROR_ID && 
            pPlayer->m_dwTeamID == m_pExceptSrc->m_dwTeamID
        );
    }
    ...
    if (m_pExceptDst)
    {
        ...
        KG_PROCESS_SUCCESS(
            pPlayer->m_dwTeamID != ERROR_ID &&
            pPlayer->m_dwTeamID == m_pExceptDst->m_dwTeamID
        );
    }
...
BOOL KBroadcastTalkFunc::operator()(KPlayer* pPlayer)
{
    ...
    if (m_pTalker)
    {
        pBlackNode = g_pSO3World->m_FellowshipMgr.GetBlackListNode(pPlayer->m_dwID, m_pTalker->m_dwID);
