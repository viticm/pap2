...
int KAuctionClient::LuaApplyLookup(Lua_State* L)
{
    ...
    KGLOG_PROCESS_ERROR(nTopIndex == 15);
    ...
    dwBidderID      = (DWORD)Lua_ValueToNumber(L, 10);
    nTopPrice       = (int)Lua_ValueToNumber(L, 11);
    nAuction        = (int)Lua_ValueToNumber(L, 12);
    nStartIndex     = (int)Lua_ValueToNumber(L, 13);
    byOrderType     = (BYTE)Lua_ValueToNumber(L, 14);
    bDesc           = (BOOL)Lua_ValueToNumber(L, 15);
    ...
    bRetCode = g_PlayerClient.DoAuctionLookupRequest(
        ...
        dwBidderID, nTopPrice, nAuction, nStartIndex, byOrderType, bDesc
    );
...
int KAuctionClient::LuaSell(Lua_State* L)
{
    ...
    bRetCode = g_PlayerClient.DoAuctionSellRequest(dwNpcID, dwBox, dwX, nStartPrice, nBuyItNowPrice, nLeftHour);
    KGLOG_PROCESS_ERROR(bRetCode);
