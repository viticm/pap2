...
BOOL KCharacter::Init(void)
{
    ...
    m_nCriticalDamagePowerBaseKiloNumRate   = 0;
    m_nCriticalDamagePower                  = 0;
    m_nCriticalDamagePowerBase              = 0;
    m_nCriticalDamagePowerPercent           = KILO_NUM;
...
BOOL KCharacter::CheckDie()
{
	...
	{        
		...
		pPlayer->m_ReviveParam.eState = crsWaitingForRevive;

        if (pPlayer->m_ReviveParam.nLastSituReviveTime > 0)
        {
		    if (g_pSO3World->m_nCurrentTime - pPlayer->m_ReviveParam.nLastSituReviveTime >= SITU_REVIVE_INTERVAL)
		    {
			    pPlayer->m_ReviveParam.nSituReviveCount = 0;
		    }
        }
	    
		KGLOG_PROCESS_ERROR(pPlayer->m_ReviveParam.nSituReviveCount >= 0);
        KGLOG_PROCESS_ERROR(pPlayer->m_ReviveParam.nSituReviveCount < MAX_DEATH_PUNISH_TIME_SIZE);
	    
        if (m_pScene->m_nReviveCycle > 0)
        {
            nSituReviveFrame = 
                g_pSO3World->m_nGameLoop + (m_pScene->m_nReviveCycle - (g_pSO3World->m_nGameLoop % m_pScene->m_nReviveCycle));
        }
        else
        {
		    nDeathPunishTime = g_pSO3World->m_Settings.m_ConstList.nDeathPunishSecond[pPlayer->m_ReviveParam.nSituReviveCount];
            nSituReviveFrame = g_pSO3World->m_nGameLoop + nDeathPunishTime * GAME_FPS;
        }

        pPlayer->m_ReviveParam.nSituReviveFrame = nSituReviveFrame;
        pPlayer->m_ReviveParam.nReviveLife      = 0;
        pPlayer->m_ReviveParam.nReviveMana      = 0;
	...
	else
    {
        ...
        {
            pPlayer = g_pSO3World->m_PlayerSet.GetObj(pNpc->m_dwDropTargetPlayerID);
            if (pPlayer)
            {
...
#ifdef _SERVER
    g_PlayerServer.DoCharacterDeath(this, m_dwKillerID, nSituReviveFrame);
...
BOOL KCharacter::MoveTo(int nX, int nY, int nZ, BOOL bIgnoreBlock, BOOL bBroadcast)
{
    ...
    pDstCell = pDstRegion->GetProximalCellForObj(
        nDstCellX, nDstCellY, nZ, m_nHeight, false, bIgnoreBlock, &nRelativeZ
    );
...
{
    m_nMaxLife = m_nMaxLifeBase * (KILO_NUM + m_nMaxLifePercentAdd) / KILO_NUM + m_nMaxLifeAdditional;

    m_nMaxMana = m_nMaxManaBase * (KILO_NUM + m_nMaxManaPercentAdd) / KILO_NUM + m_nMaxManaAdditional;

    if (IS_PLAYER(m_dwID))
    {
        KPlayer* pPlayer = (KPlayer*)this;

        pPlayer->m_nMaxLife += pPlayer->m_nCurrentVitality * pPlayer->m_nVitalityToMaxLifeCof / KILO_NUM;
        pPlayer->m_nMaxMana += pPlayer->m_nCurrentSpirit * pPlayer->m_nSpiritToMaxManaCof / KILO_NUM;
    }

    MAKE_IN_RANGE(m_nMaxLife, 1, MAX_LIFE_VALUE);
    MAKE_IN_RANGE(m_nMaxMana, 1, MAX_MANA_VALUE);
...
BOOL KCharacter::ApplyBullet(KSKILL_BULLET *pBullet)
{
    ...
    // -------------------- 无敌判定 ---------------------------------------->>
    if (m_nPositiveShield && !pSkill->m_pBaseInfo->bIgnorePositiveShield)
    {
        bShield = true;
    }

    if (m_nNegativeShield && !pSkill->m_pBaseInfo->bIgnoreNegativeShield)
    {
        bShield = true;
    }
    ...
    *pnRetDamageValue = nDst_DeltaLife;
	...
	if (pSkillSrc && pSkillSrc != this && pSkill && nDst_DeltaLife < 0)
	{
	    ...
	    int nThreat = ((-nDst_DeltaLife) * pSkill->m_nThreatRevisePercent / KILO_NUM) * 
            (KILO_NUM + m_pSkillAttributeParam->nDamageThreatCoefficient) / KILO_NUM;
	...
	else if (pSkillSrc && pSkill && nDst_EffectiveTherapy > 0)
	{
        ...
        int nThreat = (nDst_EffectiveTherapy * pSkill->m_nThreatRevisePercent / KILO_NUM) * 
            (KILO_NUM + m_pSkillAttributeParam->nTherapy) / KILO_NUM / THERAPY_TO_THREAT;
	...
	else if (pSkillSrc && pSkillSrc != this && pSkill && nSrc_EffectiveTherapy > 0)
	{
        ...
        int nThreat = (nSrc_EffectiveTherapy * pSkill->m_nThreatRevisePercent / KILO_NUM) * 
            (KILO_NUM + m_pSkillAttributeParam->nTherapyThreatCoefficient) / KILO_NUM / THERAPY_TO_THREAT;
...
BOOL KCharacter::ProcessPull(KCharacter* pCharacter, int nVelocityXY)
{
    ...
    KG_PROCESS_SUCCESS(nApplyOdds + m_nKnockedBackRate < KILO_NUM); // 这里的免疫判断和击退一样的数值

    nDistanceCharacter = m_nTouchRange + pCharacter->m_nTouchRange + CELL_LENGTH * 2; // 两个人间隔两个CELL

    nDestX = pCharacter->m_nX + nDistanceCharacter * g_Cos(m_nDirectionXY) / SIN_COS_NUMBER;
    nDestY = pCharacter->m_nY + nDistanceCharacter * g_Sin(m_nDirectionXY) / SIN_COS_NUMBER;
...
BOOL KCharacter::ProcessRevive(int nLife, int nMana)
{
    ...
    if (pPlayer->m_ReviveParam.eState == crsConfirmRevive)
    {
        pPlayer->m_ReviveParam.eState = crsWaitingForRevive;
    }

    KGLOG_PROCESS_ERROR(pPlayer->m_ReviveParam.eState == crsWaitingForRevive);

    pPlayer->m_ReviveParam.eState       = crsConfirmRevive;
    pPlayer->m_ReviveParam.dwReviver    = pSkillSrc->m_dwID;
    pPlayer->m_ReviveParam.nReviveLife  = nLife;
    pPlayer->m_ReviveParam.nReviveMana  = nMana;

    bRetCode = pSkillSrc->GetAbsoluteCoordinate(
        &pPlayer->m_ReviveParam.nX, &pPlayer->m_ReviveParam.nY, &pPlayer->m_ReviveParam.nZ
    );
    KGLOG_PROCESS_ERROR(bRetCode);

    g_PlayerServer.DoPlayerRevivedNotiy(pPlayer->m_nConnIndex, pSkillSrc);
...
BOOL KCharacter::ProcessTherapy(int nTherapyValue)
{
    ...
    nTherapyValue = nTherapyValue * (KILO_NUM + m_pSkillAttributeParam->nDamageAddPercent) / KILO_NUM;    // 技能秘笈加成

    if (m_pSkillAttributeParam->nStackNum > 1)
    {
        nTherapyValue = nTherapyValue * m_pSkillAttributeParam->nStackNum;
    }
    ...
    if (m_pSkillAttributeParam->bCriticalStrike)
    {
        if (pCaster)
        {
            nDenominatorSrc 	= 74 * pCaster->m_nLevel + 320;
            KGLOG_PROCESS_ERROR(nDenominatorSrc > 0);

            nCriDamagePower 	= pCaster->m_nCriticalDamagePowerBaseKiloNumRate + KILO_NUM * pCaster->m_nCriticalDamagePower / nDenominatorSrc;
...
BOOL KCharacter::ProcessDamage(int nDamageType, int nDamageValue)
{
    ...
    nDamageValue = nDamageValue * (KILO_NUM + m_pSkillAttributeParam->nDamageAddPercent) / KILO_NUM;    // 技能秘笈加成
    ...
    if (pSkillSrc && IS_PLAYER(pSkillSrc->m_dwID)) // 如果攻击方是Player则计算阵营对攻击的提升
    {
        ...
        nDamageValue = nDamageValue * nCampDamagePercent / KILO_NUM;
    }
    ...
    if (IS_PLAYER(m_dwID)) // 如果被击方是Player则计算阵营对伤害减免的提升
    {
        ...
        nDamageValue = nDamageValue * nCampShieldPercent / KILO_NUM;
    }

    if (m_pSkillAttributeParam->nStackNum > 1)
    {
        nDamageValue = nDamageValue * m_pSkillAttributeParam->nStackNum;
    }
    ...
    if (pSkill)
    {
        DamageCalculateParam.nDamageToLifeForSrcParty   = pSkill->m_nDamageToLifeForParty;
        DamageCalculateParam.nDamageToManaForSrcParty   = pSkill->m_nDamageToManaForParty;
    }
...
BOOL KCharacter::CalculateDamage(const KDAMAGE_CALCULATE_PARAM &crParam)
{
    ...
    int             nDamageToLifeForSrcParty  = 0;
    int             nDamageToManaForSrcParty  = 0;
    int             nCriDamagePower           = 0;
    ...
    if (pSkillSrc)
    {
        ...
        if (dwDstMoveStateMask & pSkillSrc->m_dwAddDamageByDstMoveStateMask)
        {
            nDamageToDst = nDamageToDst * (KILO_NUM + pSkillSrc->m_nAddDamagePercentByDstMoveState) / KILO_NUM; // 目标移动状态对伤害的加成
        }
    }
    ...
    if (crParam.bCriticalStrikeFlag)
    {
        if (pSkillSrc)
        {
            nDenominatorSrc = 74 * pSkillSrc->m_nLevel + 320;
            KGLOG_PROCESS_ERROR(nDenominatorSrc > 0);

            nDenominatorDst = 74 * m_nLevel + 320;
            KGLOG_PROCESS_ERROR(nDenominatorDst > 0);

            nCriDamagePower = pSkillSrc->m_nCriticalDamagePowerBaseKiloNumRate + KILO_NUM * pSkillSrc->m_nCriticalDamagePower / nDenominatorSrc;
    ...
    *crParam.pnDstMaxTypedDamageManaShield -= nTypedManaShieldDamage;
    ...
    if (pSkillSrc && IS_PLAYER(pSkillSrc->m_dwID))
    {
        KPlayer* pPlayer        = (KPlayer*)pSkillSrc;
        // DamageToLife
        nDamageToLifeForSrcParty += nDamageToDst * crParam.nDamageToLifeForSrcParty / KILO_NUM;

        if (nDamageToLifeForSrcParty < 0)
        {
            nDamageToLifeForSrcParty = 0;
        }

        // DamageToMana
        nDamageToManaForSrcParty += nDamageToDst * crParam.nDamageToManaForSrcParty / KILO_NUM;

        if (nDamageToManaForSrcParty < 0)
        {
            nDamageToManaForSrcParty = 0;
        }
        ...
        if (pTeam)
        {
            ...
            )
            {
                ...
                nRetCode = g_GetDistance3(
                    pMemberPlayer->m_nX, pMemberPlayer->m_nY, pMemberPlayer->m_nZ,
                    m_nX, m_nY, m_nZ
                );
                ...
                pMemberPlayer->m_nCurrentLife += nDamageToLifeForSrcParty;
                pMemberPlayer->m_nCurrentMana += nDamageToManaForSrcParty;
            }
        }
        else // 没有队伍，只给自己加血
        {
            pSkillSrc->m_nCurrentLife += nDamageToLifeForSrcParty;
            pSkillSrc->m_nCurrentMana += nDamageToManaForSrcParty;
        }
    }
...
void KCharacter::CheckBeatBack()
{
    ...
    if (m_OTActionParam.eActionType == otActionSkillPrepare || m_OTActionParam.eActionType == otActionSkillChannel)
    {
        ...
        nBeatBackRate = m_nBeatBackRate + pOTSkill->m_nBeatBackRate;
    }
...
BOOL KCharacter::OnDeath(void)
{
    ...
    if (pDropTarget)
    {
        pLootList = pNpc->GenerateLootList(pDropTarget, pDoodad);
        if (pLootList)
	    {
...
BOOL KCharacter::OnItemSkill()
{
    ...
    bRetCode = g_pSO3World->m_SkillManager.GetSkillRecipeKey(&RecipeKey, dwSkillID, dwSkillLevel, this);
    KGLOG_PROCESS_ERROR(bRetCode);
    ...
    bRetCode = pPlayer->m_ItemList.GetItemPos(pItem->m_dwID, dwBox, dwBoxX);
	if (!bRetCode)
	{
        g_PlayerServer.DoSkillBeatBack(this, 0, true);
		DoActionIdle();
	...
	eRetCode = pSkill->CanCastOnPrepare(this, m_SkillTarget);
	if (eRetCode != srcSuccess)
	{
		g_PlayerServer.DoMessageNotify(pPlayer->m_nConnIndex, ectSkillErrorCode, nRetCode);
		DoActionIdle();
		goto Exit1;
	}
	...
	if (pSkill->m_pBaseInfo->bIsChannelSkill)
	{
        ...
        KSkillRecipePointer SkillRecipePointer;
		// ... ...
        SkillRecipePointer.SetRecipeKey(RecipeKey);
		DoSkillChannel(SkillRecipePointer, nChannelFrame);
	}
	else
	{
	...
	g_PlayerServer.DoSkillCast(this, pSkill, m_SkillTarget, false);

	nRetCode = pSkill->Cast(this, this, m_SkillTarget, RecipeKey);
	KGLOG_CHECK_ERROR(nRetCode == srcSuccess);

	if (pItem->m_Common.bConsumeDurabiltity)
	{
		nRetCode = pPlayer->m_ItemList.CostSingleItem(dwBox, dwBoxX, 1);
		KGLOG_CHECK_ERROR(nRetCode);
	}
...
BOOL KCharacter::OnRecipePrepare()
{
	...
	nRetCode = pCraft->Cast((KPlayer*)this, pProfessionSkill->dwRecipeID, pProfessionSkill->dwSourceItemID, pProfessionSkill->Target);
    KGLOG_CHECK_ERROR(nRetCode == srcSuccess);
#endif
...
void KCharacter::ProcessAutoFly()
{
    ...
    if (m_nMoveFrameCounter >= pTrack->nFrame)
    {
        ...
        if (pCurrentNode->dwCityID == (DWORD)m_nTargetCity)
        {
            m_nCurrentTrack = 0;
            Stop();
        ...
        if (IS_PLAYER(m_dwID)) // NPC没有开通交通的列表，没有势力和帮派的判断
        {
            ...
            bRetCode = pThis->m_ReputeList.GetReputeLevel(pCurrentNode->dwForceID);
        ...
        if (pCurrentNode->dwMapID != pToNode->dwMapID)
        {
            ...
            bRetCode = pThis->SwitchMap(pToNode->dwMapID, 0, pToNode->nX, pToNode->nY, pToNode->nZ);
            KG_PROCESS_ERROR_RET_CODE(bRetCode, afeError);
...
//         }
    ...
    if (m_pScene->m_bDoNotGoThroughRoof)
    {
        for (KCell* pCellNode = m_pRegion->GetNextCell(m_pCell, bIgnoreDynamicObstacle); pCellNode != NULL; pCellNode = m_pRegion->GetNextCell(pCellNode, bIgnoreDynamicObstacle))
        {
            ...
            {
                KG_PROCESS_ERROR(pCellNode->m_wLowLayer * POINT_PER_ALTITUDE >= pDestCell->m_wHighLayer * POINT_PER_ALTITUDE + nRelativeZ + m_nHeight / 2);
                KG_PROCESS_ERROR((pCellNode->m_wLowLayer - m_pCell->m_wLowLayer) * POINT_PER_ALTITUDE >= m_nHeight);
                break;
            }
        }
    }
