...
BOOL KRecipeList::Load(BYTE* pbyData, size_t uDataLen)
{
    BOOL            bResult         = false;
...
#ifdef _SERVER
    g_PlayerServer.DoSyncRecipeState(m_pPlayer->m_nConnIndex, pbyData, uDataLen);
#endif
...
BOOL KRecipeList::CanLearnRecipe(DWORD dwCraftID, DWORD dwRecipeID, KRecipeLearningInfo* pLearningInfo)
{
	...
	dwProfessionLevel = m_pPlayer->m_ProfessionList.GetProfessionLevel(pRecipe->dwProfessionID);
	KG_PROCESS_ERROR(dwProfessionLevel > 0);
	KG_PROCESS_ERROR(dwProfessionLevel >= pRecipe->dwRequireProfessionLevel);
