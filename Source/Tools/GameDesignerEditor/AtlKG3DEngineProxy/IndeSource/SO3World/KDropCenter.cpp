...
BOOL KDropCenter::UnInit(void)
{
	...
	m_mapDoodadClassMapID2DropList.clear();
...
BOOL KDropCenter::NpcMapDrop(KLootList* pLootList, std::vector<DWORD>& vecLooterList, DWORD dwMapID)
{
	...
	if (pItem)
	{
		bRetCode = AddItemToLootListFromNpc(pLootList, pItem, DropData, vecLooterList);
		KGLOG_PROCESS_ERROR(bRetCode);
	}
...
BOOL KDropCenter::NpcClassDrop(KLootList* pLootList, std::vector<DWORD>& vecLooterList, DWORD dwClassID, int nLevel)
{
	...
	it = m_mapClassID2DropList.find(MAKE_INT64(dwClassID, nLevel));
	KG_PROCESS_SUCCESS(it == m_mapClassID2DropList.end());
...
BOOL KDropCenter::DoodadClassDrop(KLootList* pLootList, KPlayer* pOpener, DWORD dwClassID, DWORD dwDropIndex, int nLevel)
{
    BOOL            bResult     = false;
	BOOL			bRetCode	= false;
	KItem*			pItem		= NULL;
	...
	if (pItem)
	{
		bRetCode = AddItemToLootListFromDoodad(pLootList, pItem, DropData, pOpener);
		KGLOG_PROCESS_ERROR(bRetCode);
	}
...
BOOL KDropCenter::DoodadTemplateDrop(KLootList* pLootList, KPlayer* pOpener, DWORD dwDoodadTemplateID, DWORD dwDropIndex)
{
    BOOL            bResult     = false;
	BOOL			bRetCode	= false;
	KItem*			pItem		= NULL;
...
)
{
    ...
    {
        DropQuestItemFromNpc(it->second, dwTemplateID, pDropTarget, pLootList, vecLooterList);
    }
...
BOOL KDropCenter::DoodadDropQuestItem(DWORD dwTemplateID, KPlayer* pDropTarget, KLootList* pLootList)
{
    BOOL bResult    = false;
    BOOL bRetCode   = false;
    std::multimap<DWORD, DWORD>::iterator it;
    ...
    for (; it != UpperBoundIt; ++it)
    {
        DropQuestItemFromDoodad(it->second, dwTemplateID, pDropTarget, pLootList);
    }
...
BOOL KDropCenter::NpcClassDropInit(void)
{
	...
	for (DWORD i = 0; i < dwMaxClassID; ++i)
	{
		...
		for (DWORD dwIndex = 0; dwIndex < LevelDropList.GetSize(); dwIndex++)
		{
			...
			it = m_mapClassID2DropList.find(MAKE_INT64(i, LevelDropData.nLevel));

			if (it == m_mapClassID2DropList.end())
			{
				KDropList                                           DropList;
				std::pair<MAP_INT64_2_DROP_LIST::iterator, bool>    InsRet;

				InsRet = m_mapClassID2DropList.insert(std::make_pair(MAKE_INT64(i, LevelDropData.nLevel), DropList));
				KGLOG_PROCESS_ERROR(InsRet.second);
	...
	for (MAP_INT64_2_DROP_LIST::iterator it = m_mapClassID2DropList.begin(); it != m_mapClassID2DropList.end(); ++it)
	{
...
)
{
    ...
    for (size_t i = 0; i < uPlayerCount; ++i)
	{
	    ...
	    {
		    if (
                pQuestInfo->dwEndRequireItemType[j] == pItem->m_GenParam.dwTabType && 
                pQuestInfo->dwEndRequireItemIndex[j] == pItem->m_GenParam.dwIndex
            )
            {
...
BOOL KDropCenter::AddItemToLootListFromNpc(
...
BOOL KDropCenter::AddItemToLootListFromDoodad(
...
void KDropCenter::DropQuestItemFromNpc(
...
)
{
	...
	for (int nIndex = 0; nIndex < QUEST_END_ITEM_COUNT; ++nIndex)
	{
		...
		DWORD           dwItemCount   = pQuestInfo->dwEndRequireItemAmount[nIndex];
        KLootItem*      pAddLootItem  = NULL;
		...
		unsigned uRandom = g_Random(MILLION_NUM);
		if (uRandom >= pQuestInfo->dwDropItemRate[nIndex])
		{
            continue;
        }
		...
		{	
			for (size_t i = 0; i < uNeedQuestItemLooterCount; ++i)
			{
                ...
                pItem = g_pSO3World->m_ItemManager.GenerateItem(CURRENT_ITEM_VERSION, dwTabType, dwItemIndex);
				KGLOG_PROCESS_ERROR(pItem);
                ...
                pMember->m_QuestList.RegisterPartyAssistance(dwQuestID);
			}
        }
        else
        {
            ...
            else // 其它情况下有需求的人轮流拾取
            {
                ...
                pRoundLooter->m_QuestList.RegisterPartyAssistance(dwQuestID);
            }
	    }
    }
...
void KDropCenter::DropQuestItemFromDoodad(
