...
int KAILogic::CallAction(KCharacter* pCharacter, int nActionID)
{
    ...
    pAction = GetAction(nActionID);
    if (pAction == NULL)
    {
        KGLogPrintf(KGLOG_ERR, "[AI] Invalid action call(%d) in AI(%d)", nActionID, m_nAIType);
        goto Exit0;
    }
    ...
    if (nActionKey < KAI_USER_ACTION)
    {
        ...
        nNextAction = (*PAction)(pCharacter, pAction);
    }
    else
    {
        int nLuaTopIndex = 0;
        ...
        {
            nNextAction = pAction->m_nBranch[nBranchIndex - 1];
        }
    }
...
int KAILogic::LuaNewAction(Lua_State* L)
{
    int         nResult     = 0;
    int         nRetCode    = 0;
    int         nActionID   = KAI_ACTION_BRANCH_NONE;
    ...
    KGLOG_PROCESS_ERROR(nActionID > KAI_ACTION_BRANCH_NONE);
...
int KAILogic::LuaNewState(Lua_State* L)
{
    ...
    RetPair = m_StateTable.insert(std::make_pair(nState, KAIState()));
    if (!RetPair.second)
    {
