...
BOOL KSkillRecipeList::Add(DWORD dwRecipeKey)
{
    ...
    else if ((it->dwRecipeKey & 0xFFFF0000) != (dwRecipeKey & 0xFFFF0000))
    {
        KGLOG_PROCESS_ERROR(m_SkillRecipeVector.size() < MAX_SKILL_REICPE_LIST_SIZE);
        m_SkillRecipeVector.insert(it, Node);
    }
    else if ((it->dwRecipeKey & 0x0000FFFF) < (dwRecipeKey & 0x0000FFFF))
    {
...
BOOL KSkillRecipeList::_LoadDataV1(size_t* puUsedSize, BYTE* pbyData, size_t uDataLen)
{
    ...
    for (int i = 0; i < pSkillRecipeData->wCount; i++)
    {
        ...
        Node.dwRecipeKey = pSkillRecipeData->dwRecipeKeyArray[i];
        Node.bActive = false;
        m_SkillRecipeVector.push_back(Node);
    }

    std::sort(m_SkillRecipeVector.begin(), m_SkillRecipeVector.end(), _RecipeListNodeCompare);
...
BOOL KSkillRecipeList::_LoadDataV2(size_t* puUsedSize, BYTE* pbyData, size_t uDataLen)
{
    ...
    for (int i = 0; i < pSkillRecipeData->wCount; i++)
    {
        ...
        Node.dwRecipeKey = pSkillRecipeData->RecipeArray[i].dwRecipeKey;
        Node.bActive = pSkillRecipeData->RecipeArray[i].byActive;
        m_SkillRecipeVector.push_back(Node);
    }
