...
BOOL KQuestList::LoadDailyQuest(size_t* puUsedSize, BYTE* pbyData, size_t uDataLen)
{
    ...
    KGLOG_PROCESS_ERROR(pDailyQuestData->byCount < MAX_ACCEPT_QUEST_COUNT);
    KGLOG_PROCESS_ERROR(uDataLen >= (sizeof(KDB_DAILY_QUEST_DATA) + pDailyQuestData->byCount * sizeof(KDB_DAILY_QUEST)));

    for (int i = 0; i < pDailyQuestData->byCount; ++i)
    {
...
BOOL KQuestList::SaveDailyQuest(size_t* puUsedSize, BYTE* pbyBuffer, size_t uBufferSize)
{
    ...
    for (int i = 0; i < MAX_ACCEPT_QUEST_COUNT; ++i)
    {
...
QUEST_RESULT_CODE KQuestList::CanAccept(KQuestInfo* pQuestInfo, KTarget* pTarget)
{
    ...
    if (!bShared) // 非共享的任务才需要做下面的检查
    {
        ...
        // 检测玩家身上是否有任务起始道具
	    if ((pQuestInfo->dwStartItemType != 0  || pQuestInfo->dwStartItemIndex != 0) && !bShared)
	    {
		    TARGET_TYPE eTargetType = pTarget->GetTargetType();

		    DWORD dwAmount = m_pPlayer->m_ItemList.GetItemAmountInEquipAndPackage(
                pQuestInfo->dwStartItemType, pQuestInfo->dwStartItemIndex
            );
	...
	for (int nIndex = 0; nIndex < QUEST_PARAM_COUNT; nIndex++)
	{
		if (pQuestInfo->dwOfferItemType[nIndex] != 0 || pQuestInfo->dwOfferItemIndex[nIndex] != 0)
		{
			nOfferItemCount++;
		}
	}
...
QUEST_RESULT_CODE KQuestList::CanAcceptDailyQuest(KQuestInfo* pQuestInfo, KNpc* pAcceptNpc)
{
    ...
    for (size_t i = 0; i < vecQuestID.size(); ++i)
    {
        ...
        {
            for (int j = 0; j < MAX_ACCEPT_QUEST_COUNT; ++j)
            {
...
int KQuestList::GetFinishedDailyQuestCount()
{
    ...
    for (int i = 0; i < MAX_ACCEPT_QUEST_COUNT; ++i)
    {
        ...
        if (m_DailyQuest[i].nNextAcceptTime > g_pSO3World->m_nCurrentTime)
        {
            ++nResult;
        }
    }
...
QUEST_RESULT_CODE KQuestList::Accept(KTarget* pTarget, DWORD dwQuestID, BOOL bIgnoreRules)
{
	...
	for (int nIndex = 0; nIndex < QUEST_PARAM_COUNT; nIndex++)
	{
		...
		pItem = g_pSO3World->m_ItemManager.GenerateItem(CURRENT_ITEM_VERSION, pQuestInfo->dwOfferItemType[nIndex], pQuestInfo->dwOfferItemIndex[nIndex]);
		KGLOG_PROCESS_ERROR(pItem);

        if (pItem->m_Common.nGenre == igBook)
        {
            pItem->m_nCurrentDurability = pQuestInfo->dwPresentItemAmount[nIndex];
        }
        else if (pItem->IsStackable())
		{
            pItem->SetStackNum(pQuestInfo->dwOfferItemAmount[nIndex]);
		}
	...
	//调用接受任务的脚本
	if (pQuestInfo->szScriptName[0] != '\0')
	{	
		bRetCode = g_pSO3World->m_ScriptCenter.IsScriptExist(pQuestInfo->szScriptName);
		if (bRetCode && g_pSO3World->m_ScriptCenter.IsFuncExist(pQuestInfo->szScriptName, SCRIPT_ON_ACCEPT_QUEST))
		{
			int nTopIndex = 0;
			...
			g_pSO3World->m_ScriptCenter.CallFunction(pQuestInfo->szScriptName, SCRIPT_ON_ACCEPT_QUEST, 0);
			g_pSO3World->m_ScriptCenter.SafeCallEnd(nTopIndex);
		}
	}
	
    g_LogClient.LogAcceptQuest(pQuestInfo->szName, pQuestInfo->nLevel, m_pPlayer->m_szAccount, m_pPlayer->m_szName);
...
)
{
    ...
    g_LogClient.LogFinishQuest(pQuestInfo->szName, pQuestInfo->nLevel, m_pPlayer->m_szAccount, m_pPlayer->m_szName);
	...
	{
		bRetCode = g_pSO3World->m_ScriptCenter.IsScriptExist(pQuestInfo->szScriptName);
		if (bRetCode && g_pSO3World->m_ScriptCenter.IsFuncExist(pQuestInfo->szScriptName, SCRIPT_ON_FINISH_QUEST))
		{
			int nTopIndex = 0;
			g_pSO3World->m_ScriptCenter.SafeCallBegin(&nTopIndex);
			...
			g_pSO3World->m_ScriptCenter.CallFunction(pQuestInfo->szScriptName, SCRIPT_ON_FINISH_QUEST, 0);
			g_pSO3World->m_ScriptCenter.SafeCallEnd(nTopIndex);
		}
	}
...
void KQuestList::OnKillNpc(DWORD dwTemplateID)
{
    ...
    while (QuestIt != UpperBoundIt)
    {
        ...
        RegisterPartyAssistance(m_AcceptedQuestList[nQuestIndex].pQuestInfo->dwQuestID);
...
void KQuestList::OnUpdateItem(DWORD dwType, DWORD dwIndex, BOOL bGetItem)
{
	...
	for (int nQuestIndex = 0; nQuestIndex < MAX_ACCEPT_QUEST_COUNT; nQuestIndex++)
	{
		...
		{
			if (pQuestInfo->dwEndRequireItemType[i] != dwType || pQuestInfo->dwEndRequireItemIndex[i] != dwIndex)
			{
                continue;
            }
...
QUEST_RESULT_CODE KQuestList::Cancel(int nQuestIndex)
{
	...
	{
		bRetCode = g_pSO3World->m_ScriptCenter.IsScriptExist(pQuestInfo->szScriptName);
		if (bRetCode && g_pSO3World->m_ScriptCenter.IsFuncExist(pQuestInfo->szScriptName, SCRIPT_ON_CANCEL_QUEST))
		{
			int nTopIndex = 0;
			g_pSO3World->m_ScriptCenter.SafeCallBegin(&nTopIndex);
            ...
            g_pSO3World->m_ScriptCenter.CallFunction(pQuestInfo->szScriptName, SCRIPT_ON_CANCEL_QUEST, 0);
			g_pSO3World->m_ScriptCenter.SafeCallEnd(nTopIndex);
		}
	}

    g_LogClient.LogCancelQuest(pQuestInfo->szName, pQuestInfo->nLevel, m_pPlayer->m_szAccount, m_pPlayer->m_szName);
...
QUEST_RESULT_CODE KQuestList::CanFinish(KQuestInfo* pQuestInfo, KTarget* pTarget)
{
...
#ifdef _SERVER
		bRetCode = pDoodad->CheckDistance(m_pPlayer);
		KGLOG_PROCESS_ERROR(bRetCode);
#endif
	}

	//检测玩家身上是否有任务结束道具
	if (pQuestInfo->dwEndItemType != 0  || pQuestInfo->dwEndItemIndex != 0)
	{
		DWORD dwAmount = m_pPlayer->m_ItemList.GetItemAmountInEquipAndPackage(
			pQuestInfo->dwEndItemType, pQuestInfo->dwEndItemIndex);
	...
	if (pQuest)
	{
		...
		for (int nIndex = 0; nIndex < QUEST_PARAM_COUNT; nIndex++)
		{
			...
			{
				KG_PROCESS_ERROR_RET_CODE(pQuest->dwKillNpcCount[nIndex] >= 
					pQuestInfo->dwKillNpcAmount[nIndex], qrcErrorKillNpcCount);
			}
		}
	}
...
int KQuestList::GetQuestPhase(DWORD dwQuestID)
{
	...
	{			
		...
		for (int nIndex = 0; nIndex < QUEST_PARAM_COUNT; nIndex++)
		{
			if (pQuestInfo->dwKillNpcTemplateID[nIndex] != ERROR_ID)
			{
				KG_PROCESS_ERROR(pQuest->dwKillNpcCount[nIndex] >= pQuestInfo->dwKillNpcAmount[nIndex]);
			}
		}
	}
...
#ifdef _SERVER
BOOL KQuestList::RegisterPartyAssistance(DWORD dwQuestID)
{
    BOOL        bResult     = false;
    KQuestInfo* pQuestInfo  = NULL;
    KTeam*      pTeam       = NULL;
...
BOOL KQuestList::GetQuestExpAttenuation(KQuestInfo* pQuestInfo, int* pnExp, int* pnRepeatCutPercent, int* pnLevelCutPercent)
{
	int nLevelMargin    = 0;
	...
	*pnExp = pQuestInfo->nPresentExp;

	*pnRepeatCutPercent = 100;
	if (pQuestInfo->bRepeat)
		*pnRepeatCutPercent = pQuestInfo->nRepeatCutPercent;

	if (m_pPlayer->m_nLevel >= pQuestInfo->nLevel)
	{
		nLevelMargin = min(m_pPlayer->m_nLevel - pQuestInfo->nLevel, 9);
	}
...
BOOL KQuestList::IsQuestEndItemEnoughByIndex(KQuestInfo* pQuestInfo, int nIndex, BOOL bAllPackage)
{
    ...
    dwTabType       = pQuestInfo->dwEndRequireItemType[nIndex];
    dwTabIndex      = pQuestInfo->dwEndRequireItemIndex[nIndex];
    dwRequireAmount = pQuestInfo->dwEndRequireItemAmount[nIndex];
    ...
    pItemInfo = g_pSO3World->m_ItemManager.GetItemInfo(CURRENT_ITEM_VERSION, dwTabType, dwTabIndex);
    KGLOG_PROCESS_ERROR(pItemInfo);
    
    if (pItemInfo->nGenre == igBook)
    {
        dwRequireAmount = 1;
        nBookRecipeID   = pQuestInfo->dwEndRequireItemAmount[nIndex]; // 书的ID
    }
...
BOOL KQuestList::IsQuestEndBookMemorized(KQuestInfo* pQuestInfo)
{
    ...
    for (int i = 0; i < QUEST_END_ITEM_COUNT; i++)
	{
        dwTabType       = pQuestInfo->dwEndRequireItemType[i];
        dwTabIndex      = pQuestInfo->dwEndRequireItemIndex[i];
        ...
        nBookRecipeID = pQuestInfo->dwEndRequireItemAmount[i];
...
void KQuestList::ProcessDailyQuest()
{
    for (int i = 0; i < MAX_ACCEPT_QUEST_COUNT; ++i)
    {
        ...
        if (g_pSO3World->m_nCurrentTime > m_DailyQuest[i].nNextAcceptTime)
        {
            m_DailyQuest[i].dwQuestID = 0;
            m_DailyQuest[i].nNextAcceptTime = 0;            
...
QUEST_RESULT_CODE KQuestList::CommonCheck(KQuestInfo* pQuestInfo)
{
    ...
    KG_PROCESS_ERROR_RET_CODE(pQuestInfo->nRequireCampMask & (0x1 << (int)m_pPlayer->m_eCamp), qrcErrorCamp); // 检查阵营需求
    ...
    if (pQuestInfo->bRequireReputeAll)
    {
        for (int i = 0; i < QUEST_PARAM_COUNT; i++)
        {
            ...
            if (!pQuestInfo->dwRequireForceID[i])
                continue;

            nReputeLevel = m_pPlayer->m_ReputeList.GetReputeLevel(pQuestInfo->dwRequireForceID[i]);
            KG_PROCESS_ERROR_RET_CODE(nReputeLevel >= pQuestInfo->nReputeLevelMin[i], qrcErrorRepute);
            KG_PROCESS_ERROR_RET_CODE(nReputeLevel <= pQuestInfo->nReputeLevelMax[i], qrcErrorRepute);
        }
    }
    else
    {
        BOOL bReputeMatch = false;

        for (int i = 0; i < QUEST_PARAM_COUNT; i++)
        {
            int nReputeLevel = 0;

            if (pQuestInfo->dwRequireForceID[i] == 0)
                continue;

            nReputeLevel = m_pPlayer->m_ReputeList.GetReputeLevel(pQuestInfo->dwRequireForceID[i]);
            if (nReputeLevel >= pQuestInfo->nReputeLevelMin[i] && nReputeLevel <= pQuestInfo->nReputeLevelMax[i])
            {
                bReputeMatch = true;
                break;
            }
    ...
    // 检查等级
	KG_PROCESS_ERROR_RET_CODE(
        m_pPlayer->m_nLevel >= pQuestInfo->nMinLevel, qrcTooLowLevel
    );
    
    KG_PROCESS_ERROR_RET_CODE(
        m_pPlayer->m_nLevel <= pQuestInfo->nMaxLevel, qrcTooHighLevel
    );
    
    // 检测玩家性别
	KG_PROCESS_ERROR_RET_CODE(
        pQuestInfo->nRequireGender == 0 || m_pPlayer->m_eGender == pQuestInfo->nRequireGender, 
        qrcErrorGender
    );
    
    // 检测玩家体型
	KG_PROCESS_ERROR_RET_CODE(
        pQuestInfo->nRequireRoleType == 0 || m_pPlayer->m_eRoleType == pQuestInfo->nRequireRoleType,
		qrcErrorRoleType
    );
    ...
    // 检测武功技能等级
	dwSkillLevel = m_pPlayer->m_SkillList.GetSkillLevel(pQuestInfo->dwRequireSkillID);
	KG_PROCESS_ERROR_RET_CODE(dwSkillLevel >= pQuestInfo->dwMinSkillLevel, qrcTooLowSkillLevel);

	// 检测生活技能等级
	if (pQuestInfo->dwRequireLifeSkillID)
	{
		DWORD dwProfessionLevel = m_pPlayer->m_ProfessionList.GetProfessionLevel(
			pQuestInfo->dwRequireLifeSkillID
        );
		KG_PROCESS_ERROR_RET_CODE(
            dwProfessionLevel >= pQuestInfo->dwMinLifeSkillLevel, qrcTooLowLifeLevel
        );
	}
	...
	{
		if (
            pQuestInfo->dwStartRequireItemType[nIndex] != 0 || pQuestInfo->dwStartRequireItemIndex[nIndex] != 0
        )
		{
			DWORD dwAmount = m_pPlayer->m_ItemList.GetItemAmountInEquipAndPackage(
				pQuestInfo->dwStartRequireItemType[nIndex], pQuestInfo->dwStartRequireItemIndex[nIndex]
            );
			KG_PROCESS_ERROR_RET_CODE(
                dwAmount >= pQuestInfo->dwStartRequireItemAmount[nIndex], qrcNotEnoughStartItem
            );
		}
	}
...
QUEST_RESULT_CODE KQuestList::Present(KQuestInfo* pQuestInfo, int nPresentChoice1, int nPresentChoice2)
{
    ...
    if (
        ...
        (pQuestInfo->bRepeat && pQuestInfo->nRepeatCutPercent == 100)
    )
    {
    ...
    {
        nPresentPerent = pQuestInfo->nRepeatCutPercent;
    }

	// 增加声望奖励
	for (nIndex = 0; nIndex < QUEST_PARAM_COUNT; nIndex++)
	{
		if (pQuestInfo->dwAffectForceID[nIndex] && pQuestInfo->nAffectForceValue[nIndex])
		{
			bRetCode = m_pPlayer->m_ReputeList.ModifyReputation(
                pQuestInfo->dwAffectForceID[nIndex], pQuestInfo->nAffectForceValue[nIndex]
            );
    ...
    if (nLevelMargin < 4)
    {
        nAddExp = pQuestInfo->nPresentExp;
    }
    else
    {
        nAddExp = pQuestInfo->nPresentExp * (85 - (nLevelMargin - 4) * 15) / 100;
    }

	m_pPlayer->AddExp(nAddExp);

	// 奖励技能
	if (pQuestInfo->nPresentSkill > 0)
	{
        KSkill* pSkill = NULL;

        pSkill = g_pSO3World->m_SkillManager.GetSkill_RAW(pQuestInfo->nPresentSkill, 1);
        if (pSkill == NULL)
        {
            KGLogPrintf(
                KGLOG_DEBUG, 
		...
		m_pPlayer->m_SkillList.LearnSkill(pQuestInfo->nPresentSkill, true);
	}
    ...
    nAddMoney = (pQuestInfo->nPresentMoney * nPresentPerent / 100) * nMoneyPercent / KILO_NUM;
    ...
    // 获得成就
    if (pQuestInfo->nAchievementID >= 0 && pQuestInfo->nAchievementID < MAX_ACHIEVEMENT_ID)
    {
        m_pPlayer->AcquireAchievement(pQuestInfo->nAchievementID);
    }
...
BOOL KQuestList::CheckFreeRoomForPresent(KQuestInfo* pQuestInfo)
{
	...
	for (int nIndex = 0; nIndex < QUEST_PARAM_COUNT; nIndex++)
	{
		if (pQuestInfo->dwPresentItemType[nIndex] == 0 || pQuestInfo->dwPresentItemIndex[nIndex] == 0) 
            continue;
	...
	for (int nIndex = 0; nIndex < QUEST_PARAM_COUNT; nIndex++)
	{
		if (pQuestInfo->dwPresentItemType[QUEST_PARAM_COUNT + nIndex] == 0)
            continue;

        if (pQuestInfo->dwPresentItemIndex[QUEST_PARAM_COUNT + nIndex] == 0)
            continue;
...
QUEST_RESULT_CODE KQuestList::CanPresentItems(KQuestInfo* pQuestInfo, int nPresentChoice1, int nPresentChoice2)
{
		...
		for (int nIndex = 0; nIndex < QUEST_PARAM_COUNT; nIndex++)
		{
			if (pQuestInfo->dwPresentItemType[nIndex] != 0 || pQuestInfo->dwPresentItemIndex[nIndex] != 0)
			{
                eRetCode = CanPresentItem(
                    pQuestInfo->dwPresentItemType[nIndex], pQuestInfo->dwPresentItemIndex[nIndex], 
                    pQuestInfo->dwPresentItemAmount[nIndex]
                );
                KG_PROCESS_ERROR_RET_CODE(eRetCode == qrcSuccess, eRetCode);
			}
		}
	...
	{
		if (pQuestInfo->dwPresentItemType[nPresentChoice1] != 0 || pQuestInfo->dwPresentItemIndex[nPresentChoice1] != 0)
		{
			eRetCode = CanPresentItem(
                pQuestInfo->dwPresentItemType[nPresentChoice1], pQuestInfo->dwPresentItemIndex[nPresentChoice1], 
                pQuestInfo->dwPresentItemAmount[nPresentChoice1]
            );
            KG_PROCESS_ERROR_RET_CODE(eRetCode == qrcSuccess, eRetCode);
		}
	}
	...
	{
		if (pQuestInfo->dwPresentItemType[nPresentChoice2] != 0 || pQuestInfo->dwPresentItemIndex[nPresentChoice2] != 0)
		{
			eRetCode = CanPresentItem(
                pQuestInfo->dwPresentItemType[nPresentChoice2], pQuestInfo->dwPresentItemIndex[nPresentChoice2], 
                pQuestInfo->dwPresentItemAmount[nPresentChoice2]
            );
            KG_PROCESS_ERROR_RET_CODE(eRetCode == qrcSuccess, eRetCode);
		}
	}
...
QUEST_RESULT_CODE KQuestList::CanPresentItem(DWORD dwTabType, DWORD dwIndex, int nAmount)
{
    ...
    pItemInfo = g_pSO3World->m_ItemManager.GetItemInfo(CURRENT_ITEM_VERSION, dwTabType, dwIndex);
    KGLOG_PROCESS_ERROR(pItemInfo);
...
BOOL KQuestList::CostQuestRequireItem(KQuestInfo* pQuestInfo)
{
	...
	for (int nIndex = 0; nIndex < QUEST_END_ITEM_COUNT; nIndex++)
	{
        ...
        dwItemType	    = pQuestInfo->dwEndRequireItemType[nIndex];
        dwItemIndex     = pQuestInfo->dwEndRequireItemIndex[nIndex];
        dwCostAmount    = pQuestInfo->dwEndRequireItemAmount[nIndex];
        ...
        pItemInfo = g_pSO3World->m_ItemManager.GetItemInfo(CURRENT_ITEM_VERSION, dwItemType, dwItemIndex);
        KGLOG_PROCESS_ERROR(pItemInfo);

		for (nIndex2 = 0; nIndex2 < nIndex; nIndex2++)
		{
			if (
                dwItemType == pQuestInfo->dwEndRequireItemType[nIndex2] && 
                dwItemIndex == pQuestInfo->dwEndRequireItemIndex[nIndex2]
            )
                break;
		}
        ...
        {
            dwCostAmount = 1;
            nBookRecipeID = pQuestInfo->dwEndRequireItemAmount[nIndex];
        }
...
BOOL KQuestList::CostQuestOfferItem(KQuestInfo* pQuestInfo)
{
	...
	for (int nIndex = 0; nIndex < QUEST_PARAM_COUNT; nIndex++)
	{
		...
		DWORD dwTabIndex    = pQuestInfo->dwOfferItemIndex[nIndex];
        ...
        dwCostAmount = pQuestInfo->dwOfferItemAmount[nIndex];
...
BOOL KQuestList::RegisterQuestAssistInfo(DWORD dwPlayerID, DWORD dwQuestID)
{
    BOOL            bResult         = false;
    BOOL            bRetCode        = 0;
    KPlayer*        pAssister       = NULL;
    ...
    eQuestState = pAssister->m_QuestList.GetQuestState(dwQuestID);
    KG_PROCESS_ERROR(eQuestState == qsFinished);

    bRetCode = g_InRange(m_pPlayer, pAssister, MAX_QUEST_ASSIST_DISTANCE);
    KG_PROCESS_ERROR(bRetCode);

    pLevelUpData = g_pSO3World->m_Settings.m_LevelUpList.GetLevelUpData(pAssister->m_eRoleType, pAssister->m_nLevel);
    KGLOG_PROCESS_ERROR(pLevelUpData);

    KG_PROCESS_ERROR(pAssister->m_QuestList.m_nAssistDailyCount < pLevelUpData->nMaxAssistTimes);
