...
const char* KSmartDialogList::GetDialogText(DWORD dwDialogID, NPC_TALK_ACTION_TYPE eAction, DWORD dwIndex)
{
	KNpcTemplate* pNpcTemplate = NULL;
	...
	nKey = MAKE_INT64(dwDialogID, (int)eAction);
	iter = m_mapSmartDialogList.find(nKey);
	if (iter != m_mapSmartDialogList.end())
	{
		pNpcDialog = &(iter->second);
		szText = pNpcDialog->szDialogText[dwIndex];
	}
