...
void KRelayClient::OnTeamAddMemberNotify(BYTE* pbyData, size_t uDataLen)
{
    ...
    pLeaderInfo = g_pSO3World->m_TeamServer.GetMember(pPartyAddMemberNotify->dwPartyID, pTeam->dwAuthority[tatLeader]);
    ...
    if (pLeaderInfo && pPartyMember) // if Team had leader
    {
    ...
    else if (pPartyMember) // New Team, You are Leader
    {
        g_PlayerServer.DoMessageNotify(
            pPartyMember->m_nConnIndex, ectPartyNotifyCode, epncPartyCreated,
            0, 0
        );
...
void KRelayClient::OnTeamChangeMemberGroupNotify(BYTE* pbyData, size_t uDataLen)
{
    ...
    bRetCode = g_pSO3World->m_TeamServer.ChangeMember(
        pNotify->dwTeamID, pNotify->nOldGroupIndex, pNotify->nNewGroupIndex, pNotify->dwMemberID
    );
    KGLOG_PROCESS_ERROR(bRetCode);
...
BOOL KRelayClient::DoTeamChangeMemberGroupRequest(DWORD dwTeamID, DWORD dwMemberID, int nDstGroupIndex, DWORD dwRequestSrc)
{
    BOOL                                    bResult     = false;
    BOOL                                    bRetCode    = false;
    IKG_Buffer*                             piSendBuff  = NULL;
...
BOOL KRelayClient::DoSyncMidMapMark(KPlayer* pSrc, int nMap, int nX, int nY, int nType, const char* pszComment)
{
	...
	KG_PROCESS_ERROR(pSrc->m_dwTeamID != ERROR_ID);
