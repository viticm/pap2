...
TRADING_RESULT_CODE KTradingBox::AddItem(DWORD dwGridIndex, DWORD dwBoxIndex, DWORD dwPosIndex)
{
    ...
    bRetCode = CheckItemExistLimit(pItem->m_GenParam.dwTabType, pItem->m_GenParam.dwIndex, pItem->m_GenParam.nVersion, nStackNum);
...
BOOL KTradingBox::FinishTradingCheck()
{
    ...
    for (DWORD dwIndex = 0; dwIndex < MAX_TRADING_BOX_SIZE; ++dwIndex)
    {
        ...
        // 检查物品拥有限制
        bRetCode = CheckItemExistLimit(pItem->m_GenParam.dwTabType, pItem->m_GenParam.dwIndex, pItem->m_GenParam.nVersion, 0);
    ...
    for (DWORD dwIndex = 0; dwIndex < MAX_TRADING_BOX_SIZE; ++dwIndex)
    {
        ...
        bRetCode = pOtherTradingBox->CheckItemExistLimit(
            pItem->m_GenParam.dwTabType, pItem->m_GenParam.dwIndex, pItem->m_GenParam.nVersion, 0
        );
        KG_PROCESS_ERROR(bRetCode);
    }
...
#ifdef _SERVER
BOOL KTradingBox::CheckItemExistLimit(DWORD dwTabType, DWORD dwItemIndex, int nVersion, int nItemAmount)
{
    BOOL        bResult             = false;
    KItemInfo*  pItemInfo           = NULL;
    int         nItemMaxExistAmount = 0;
    ...
    pItemInfo = g_pSO3World->m_ItemManager.GetItemInfo(nVersion, dwTabType, dwItemIndex);
