...
BOOL KBinTab::SaveBinTab(LPCSTR szFilePath)
{
	...
	KGLOG_PROCESS_ERROR(!g_IsFileExist(szFilePath));
	...
	pFile = g_OpenFile(szFilePath, FALSE, TRUE);
	KGLOG_PROCESS_ERROR(pFile);

	nWritten = (size_t)pFile->Write(m_pData, (unsigned long)pHeader->nDataSize);
	KGLOG_PROCESS_ERROR(nWritten == pHeader->nDataSize);
...
Exit0:
	...
	if (!bResult)
	{
		...
		KGLogPrintf(KGLOG_ERR, "SO3UI KBinTab::LoadTxtTab(%s, %s)\n", szFilePath, szFormat);
	}
