...
public:
    ...
    int Init(KCharacter* pCharacter, KRLScene* pRLScene);
    int Reset();
    ...
    int UpdateAnimationDown(RL_ANIMATION_DOWN nAnimationDown, RL_ADVANCE nAdvance, RL_FIGHT_FLAG nFightFlag, DWORD dwSkillBuffs[SKILL_BUFF_PART_COUNT]);
...
private:
    ...
    HRESULT LoadModel(KRLCharacterEquipResource const& EquipResource, BOOL bForceUpdate);
    HRESULT LoadSocket(int nIndex, KRLSocketResource const SocketParam, int nColorChannelTable);
    ...
    int UpdateAnimation(RL_ANIMATION_DOWN nAnimationDown, RL_ANIMATION_STATE nAnimationState, RL_ADVANCE nAdvance,
        RL_FIGHT_FLAG nFightFlag, DWORD dwSkillBuffs[SKILL_BUFF_PART_COUNT]);
...
public:
    ...
    KRLCharacterFrameData m_FrameData;
