...
int RegisterSkillEffectResult(KRLCharacter* pRLTarget, KRepresentSkillEffectResult const* pResult)
{
    ...
    dwTargetID = pRLTarget->m_FrameData.m_pCharacter->m_dwID;
...
int PlaySkillEffectResult(KRLCharacter* pRLTarget, KRepresentSkillEffectResult* pResult)
{
    ...
    if (GetSkillEffectID(dwEffectIDs, nEffectIndices, CHARACTER_SFX_SKILL_EFFECT_COUNT, pResult->Caster, pResult->dwSkillID, pResult->dwSkillLevel))
    {
        pRLTarget->PlaySkillEffectResultSFX(dwEffectIDs, pResult->Caster.dwCharacterID);
    }
