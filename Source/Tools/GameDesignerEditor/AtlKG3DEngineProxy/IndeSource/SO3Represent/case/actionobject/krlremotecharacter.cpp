...
int KRLRemoteCharacter::Update(double fTime, double fTimeLast, DWORD dwGameLoop, BOOL bFrame)
{
    ...
    {
        if (!m_RLCharacter.IsFadeOut() && m_RLCharacter.m_FrameData.m_pCharacter)
        {
            if (bFrame)
                m_RLCharacter.m_FrameData.UpdateRepresentFrameData(m_RLCharacter.m_RenderData.IsVisible());
        ...
        if (bFrame)
        {
            ...
            if (!IS_PLAYER(m_RLCharacter.GetObjectID()))
            {
                ...
                else if (nAnimationDown != RL_ANIMATION_DOWN_DEATH && bSelectable)
                {
...
int KRLRemoteCharacter::UpdatePosition()
{
    int nRetCode = false;
    int nResult = false;
...
int KRLRemoteCharacter::UpdateYaw(RL_ADVANCE *pnAdvance)
{
    ...
    m_RLCharacter.m_FrameData.UpdateCurrentDirection();

    nAdvance = GetAdvance();

    m_fFaceExpectYaw = m_RLCharacter.m_FrameData.m_Current.fFaceDirection;

    m_fFootExpectYaw = m_RLCharacter.m_FrameData.m_Current.fSubjectiveDirection;
...
RL_ADVANCE KRLRemoteCharacter::GetAdvance()
{
	...
	switch (m_RLCharacter.m_FrameData.m_Current.nMoveState)
    {
	case cmsInvalid:
	case cmsOnStand:
		 break;
...
void KRLRemoteCharacter::UpdateLifeState()
{
    int nRetCode = false;
    HRESULT hr = E_FAIL;
    ...
    if (!IS_PLAYER(m_RLCharacter.GetObjectID()))
    {
        KNpc* pNPC = (KNpc*)m_RLCharacter.m_FrameData.m_pCharacter;
