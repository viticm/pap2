...
HRESULT KRLRides::LoadMDL()
{
    ...
    fPlayerScale = GetPlayerBaseModelScale(m_pRLCharacter->m_EquipResource);

    hr = m_RenderData.SetMDLVisible(TRUE);
    KGLOG_COM_PROCESS_ERROR(hr);

    hr = m_RenderData.LoadMDL(pRidesModel->szMDLFilePath, pRidesModel->fModelScale * fPlayerScale, pRidesModel->nColorChannelTable, pRidesModel->nColorChannel); // TODO: 马还没装备偏色
    KGLOG_COM_PROCESS_ERROR(hr);
...
HRESULT KRLRides::LoadDefaultSocket()
{
    ...
    nRetCode = GetRidesSocketModel(m_pRLCharacter->m_EquipResource, m_dwRepresentID, SocketParam);
...
HRESULT KRLRides::LoadSocket(int nIndex, KRLSocketResource const SocketParam)
{
    ...
    {   
        hr = m_RenderData.LoadSocket(nIndex, SocketParam.szSocketName, SocketParam.szMeshFilePath, SocketParam.szMtlFilePath, SocketParam.fMeshScale, 0, 0);
        KGLOG_COM_PROCESS_ERROR(hr);
    }
