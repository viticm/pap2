...
#define ENABLE_PLAY_ANIMATION 1
...
HRESULT KModel::PlayMonoAnimation(IKG3DModel* p3DModel, KRLMonoAnimationParam Param)
{
    ...
    {
        cprintf("PlayMonoAnimation(%.8x, playtype:%d, %s, speed:%f, offsettime:%d, tweentime:%u, 0x%.8x, track:%d)\n", p3DModel, 
...
HRESULT KModel::PlaySplitAnimation(IKG3DModel* p3DModel, KRLSplitAnimationParam Param)
{
    ...
    if (g_pRL->m_CurrentTraceOption.bTrace3DAnimation)
    {
        cprintf("PlaySplitAnimation(%.8x, split:%d, extra:%u, playtype:%u, speed:%f, offsettime:%d, tweentime:%u, 0x%.8x, track:%u, %s)\n", 
