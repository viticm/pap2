...
HRESULT KGameWorldHandler::Activate(DWORD dwTickCount, DWORD dwGameLoop, BOOL bFrame)
{
    int nRetCode = false;
    HRESULT hr = E_FAIL;
    HRESULT hrResult = E_FAIL;
	
	KG_USE_ARGUMENT(dwTickCount);

    KGLOG_PROCESS_ERROR(g_pRL->m_pSO3World);

	double fTime = KG_GetTickCount();
    ...
    g_pRL->m_SceneMgr.Activate(fTime, g_pRL->m_fFrameTimeLast, dwGameLoop, bFrame);
	...
	if (bFrame)
    {
        KRLCharacter* pRLCharacter = GetRLCharacter(g_pRL->m_pSO3World->m_dwClientPlayerID);
        if (pRLCharacter)
        {
            // update region info
            ...
            if ((g_pRL->m_dwGameLoop % RL_UPDATE_EGION_INTERVAL) == 0)
            {
                ...
                if (D3DXVec3Length(&(vPosition - g_pRL->m_SceneMgr.m_vClientPlayerRegionPosition)) > 20.0f)
                {
                    ...
                    KGLOG_PROCESS_ERROR(pRLCharacter->m_FrameData.m_pCharacter);
                    KGLOG_PROCESS_ERROR(pRLCharacter->m_FrameData.m_pCharacter->m_pScene);

                    dwSceneID = pRLCharacter->m_FrameData.m_pCharacter->m_pScene->m_dwID;
...
void KGameWorldHandler::OnResetFrameData(int nGameLoop, DWORD dwStartTime)
{
    ...
    g_pRL->m_fFrameTimeStart = (double)dwStartTime;
}
...
void KGameWorldHandler::OnAddCharacter(KCharacter* pCharacter)
{
    ...
    DWORD dwObjectID = pCharacter->m_dwID;
    DWORD dwSceneID = pCharacter->m_pScene->m_dwID;
    DWORD dwType = IS_PLAYER(dwObjectID) ? RL_OBJECT_PLAYER : RL_OBJECT_NPC;
    ...
    if (bCurrentPlayer)
    {
        if (pRLScene->m_pRLCamera)
        {
            ...
            nRetCode = RLCharacter.Init(pCharacter, pRLScene);
            KGLOG_PROCESS_ERROR(nRetCode);
    ...
    else
    {
        ...
        pRLRemoteCharacter->m_RLCharacter.Init(pCharacter, pRLScene);
...
void KGameWorldHandler::OnRemoveCharacter(KCharacter* pCharacter, BOOL bKilled)
{
    ...
    else
    {
        ...
        if (pRLRemoteCharacter->m_RLCharacter.m_pRLVehicle)
        {
            nRetCode = pRLRemoteCharacter->m_RLCharacter.UnloadRides(
                &pRLRemoteCharacter->m_RLCharacter.m_pRLVehicle);
            KGLOG_PROCESS_ERROR(nRetCode);
        }

        if (pRLRemoteCharacter->m_RLCharacter.m_pRLHorse)
        {
            nRetCode = pRLRemoteCharacter->m_RLCharacter.UnloadRides(
                &pRLRemoteCharacter->m_RLCharacter.m_pRLHorse);
            KGLOG_PROCESS_ERROR(nRetCode);
        }

        if (pRLRemoteCharacter->m_RLCharacter.m_FrameData.m_pCharacter->m_eMoveState == cmsOnDeath)
        {
        ...
        pRLRemoteCharacter->m_RLCharacter.m_FrameData.m_pCharacter = NULL;
    }
...
HRESULT KGameWorldHandler::SetSceneObjectTitleEffect(DWORD dwScene, DWORD dwObjectType, DWORD dwObject, DWORD dwEffectID)
{
    ...
    KG_PROCESS_SUCCESS(!pRLCharacter->m_RenderData.HasLoadedMDL());
...
void KGameWorldHandler::OnCharacterBeginPickDoodad(KCharacter* pCharacter, KDoodad* pDoodad)
{
    ...
    nRetCode = pRLCharacter->PlayAnimation(pDoodadModel->dwPlayerBeginPickAnimationID, 
        RL_ANIMATION_STATE_HOLD_ONCE, RL_ANIMATION_UP_BEGIN_PICK_DOODAD, RL_ANIMATION_DOWN_UNKNOWN);
    KGLOG_PROCESS_ERROR(nRetCode);
...
void KGameWorldHandler::OnCharacterBeginCraftAnimation(KCharacter* pCharacter, int nCraftID)
{
    ...
    nRetCode = pRLCharacter->m_FrameData.UpdateRepresentFrameData(TRUE);
    KGLOG_PROCESS_ERROR(nRetCode);
...
void KGameWorldHandler::OnAddDoodad(KDoodad* pDoodad)
{
    ...
    if (pDoodad->m_eKind == dkCorpse)
    {
        ...
        pRLRemoteCharacter = pRLScene->m_CharacterGCMgr.Lookup(
            pDoodad->m_nX, 
            pDoodad->m_nY, 
            pDoodad->m_nZ, 
            pDoodad->m_nFaceDirection,
        ...
        if (pRLRemoteCharacter)
        {
            ...
            hr = pRLScene->m_CharacterGCMgr.Remove(pRLRemoteCharacter->m_RLCharacter.GetObjectID());
            KGLOG_COM_PROCESS_ERROR(hr);
...
#if ENABLE_REPRESENT_OBJECT_LOAD
	KGLogPrintf(KGLOG_INFO, "%u OnAddDoodad(%s, %u, %d, %d, %d) end %u %x\n", 
		g_pRL->m_dwGameLoop, 
		pDoodad->m_szName, pDoodad->m_dwID, pDoodad->m_nX, pDoodad->m_nY, pDoodad->m_nZ, KG_GetTickCount() - dwTickCount, pDoodad->m_pRepresentObject);
#endif
    ...
    case RL_OBJECT_NPC:
        {
            ...
            if (pRLRemoteCharacter)
            {
                ...
                pRLCharacter = &pRLRemoteCharacter->m_RLCharacter;

                hr = pRLCharacter->m_RenderData.SetMDLSelectable(bSelectable);
                KGLOG_COM_PROCESS_ERROR(hr);

                if (pRLCharacter->m_pRLHorse)
                {
                    hr = pRLCharacter->m_pRLHorse->m_RenderData.SetMDLSelectable(bSelectable);
                    KGLOG_COM_PROCESS_ERROR(hr);
                }

                if (pRLCharacter->m_pRLVehicle)
                {
                    hr = pRLCharacter->m_pRLVehicle->m_RenderData.SetMDLSelectable(bSelectable);
                    KGLOG_COM_PROCESS_ERROR(hr);
                }
            }
        }
...
HRESULT KGameWorldHandler::AttachTargetSceneObject(DWORD dwTarget, DWORD dwObject, DWORD dwObjectType)
{
	...
	hr = pTarget->AttachSceneObject(dwObject, dwObjectType);
	KGLOG_COM_PROCESS_ERROR(hr);
...
HRESULT KGameWorldHandler::DetachTargetSceneObject(DWORD dwTarget, DWORD dwObject, DWORD dwObjectType)
{
	...
	hr = pTarget->DetachSceneObject(dwObject, dwObjectType);
	KGLOG_COM_PROCESS_ERROR(hr);
...
HRESULT KGameWorldHandler::GetSceneObjectData(DWORD dwScene, DWORD dwObjectType, DWORD dwObject, KRepresentSceneObjectData* pData)
{
    return E_NOTIMPL;
}
...
HRESULT KGameWorldHandler::ExecuteCommand(LPCTSTR szCommand)
{
    int nRetCode = false;
    HRESULT hrResult = E_FAIL;
    ...
    nRetCode = ExecuteRLCommand(szCommand);
