    ...
    BOOL bOnFloor, BOOL bOnSlip, BOOL bLogicalAdjust, int nMaxAltitudeReviseInCM)
{
    ...
    if (bOnFloor)
    {
        ...
        if (bLogicalAdjust)
        {
            ...
            if (fOffsetY > 64.0f && !bOnSlip || fOffsetY > 100.0f)
            {
...
int GetTerrainSlopeLean(IKG3DScene* p3DScene, D3DXVECTOR3 vPosition, float fYawLH, RL_TERRAIN_SLOPE nTerrainSlope, float& fPitchLH, float& fRollLH)
{
    ...
    {
        vBack.x = vCenter.x - cos(fYawRH);
        vBack.y = vCenter.y;
        vBack.z = vCenter.z - sin(fYawRH);
        ...
        fPitchLH = atan2f(fPitchHeight, 1.0f);
    }

    if (nTerrainSlope & RL_TERRAIN_SLOPE_TRANSVERSE)
    {
        vLeft.x = vCenter.x + sin(fYawRH);
        vLeft.y = vCenter.y;
        vLeft.z = vCenter.z - cos(fYawRH);
        ...
        fRollLH = atan2f(fRollHeight, 1.0f);
    }
