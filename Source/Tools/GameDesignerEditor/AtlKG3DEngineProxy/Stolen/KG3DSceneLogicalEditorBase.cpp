...
{
	m_dwCurrentScriptID = GetIDFromFileName(lpstrFileName,TRUE);
}
...
{
	// c#调用的控件（应该是标准Windows Gdi）传过来的color是 argb，老场景编辑器则是abgr
	m_d3dclrCurrColor = ColorValue(Colorref2value_GDI(clr, 0x88));
}
...
HRESULT KG3DSceneLogicalEditorBase::SelectLogicalCells(int nXStart,int nZStart,IEKG3DSceneSceneEditor* pEditor,D3DXVECTOR3* pCameraPos,D3DXVECTOR3* pCameraDirct,D3DXVECTOR3 vGroundInter)
{
  ...
  //      D3DXVECTOR3 src = *pCameraPos;
  //      D3DXVECTOR3 dst = *pCameraDirct;

  //      D3DXVec3Normalize(&dst, &dst);
  //      dst = src + dst * 20000.f;

  //      pCamera->GetCrossPosition(dst, src, &Inter);
