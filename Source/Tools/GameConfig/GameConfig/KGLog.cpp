...
int KGLogInit(const KGLOG_PARAM& cLogParam, void *pvContext)
{
    ...
    KGLOG_PROCESS_ERROR(cLogParam.szPath[0]  != '\0');
    KGLOG_PROCESS_ERROR(cLogParam.szIdent[0] != '\0');
    ...
    if (gs_LogParam.Options & KGLOG_OPTION_FILE)
    {
        ...
        KGLOG_PROCESS_ERROR(nRetCode < PATH_MAX);
        ...
        KGLOG_PROCESS_ERROR((nRetCode != -1) && (nRetCode < (sizeof(gs_szLogFileNamePrefix) - 1)));
    }
