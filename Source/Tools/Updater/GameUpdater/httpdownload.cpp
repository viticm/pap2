...
#define READ_BUFFER_SIZE	(128 * 1024)				// 缓冲大小 10K
...
INT CHttpDownload::Download(LPCTSTR lpszDownloadUrl,LPCTSTR lpszSavePath /*= NULL*/,BOOL bForceDownload /*= FALSE*/,BOOL bSaveResponse /*= TRUE*/,INT nVerb /*= HTTP_VERB_GET*/,LPCTSTR lpszData /*= NULL*/,LPTSTR lpszFtpURL /*= NULL*/ )
{
	...
	while( bSendRequestAgain )
	{
		...
		do
		{
			...
			if (m_pNotify && m_dwFileDownloadedSize - dwLastNotifySize > 64 * 1024)
			{
				m_pNotify->OnProgress(m_dwFileSize, m_dwFileDownloadedSize, nRet);
				dwLastNotifySize = m_dwFileDownloadedSize;
			}
	...
	if (m_pNotify)
		m_pNotify->OnProgress(m_dwFileSize, m_dwFileDownloadedSize, 0);
...
//	返回值：VOID
////////////////////////////////////////////////////////////////////////////////
VOID CHttpDownload::SetNotifyWnd(KHttpFile* pNotify)
{
...
//	返回值：BOOL
////////////////////////////////////////////////////////////////////////////////
BOOL CHttpDownload::GetDownloadFileStatus(IN LPCTSTR lpszDownloadUrl, LPCTSTR lpszSavePath, OUT DWORD &dwFileSize, /*, OUT CTime &FileTime*/ OUT DWORD &dwDownloadedSize)
{
	// 检验要下载的URL是否为空
	m_strDownloadUrl = lpszDownloadUrl;
	m_strDownloadUrl.TrimLeft();
	...
	CString strSavePath =  lpszSavePath;
	strSavePath.TrimLeft();
	strSavePath.TrimRight();

	strSavePath += m_strFileName;

	if (strSavePath.Right(1) != '\\' )	// 只有路径，没有文件名
	{
		CString strTempSavePath = strSavePath;

		CFileStatus fileDownStatus;
		if (CFile::GetStatus(strTempSavePath, fileDownStatus))
		{
			if (m_dwDownloadSize == (DWORD)fileDownStatus.m_size)
				dwDownloadedSize = (DWORD)fileDownStatus.m_size;
			else
			{
				dwDownloadedSize = 0;
				_tremove(strTempSavePath);
			}
		}
		else
		{
