...
BOOL MySQL_CheckCharacterSetEqual(MYSQL* pSrcHandle, MYSQL* pDstHandle)
{
    ...
    int         nRetCode        = 0;
    char        szSQL[]         = "show variables like \'character_set_database\'";
    MYSQL_RES*  pSrcResultSet   = NULL;
    MYSQL_RES*  pDstResultSet   = NULL;
    MYSQL_ROW   SrcRow          = NULL;
    MYSQL_ROW   DstRow          = NULL;
    unsigned    uFieldCount     = 0;
    const char* pcszSrcCharset  = NULL;
    const char* pcszDstCharset  = NULL;
    ...
    // get database character set
    // ---------------- source DB--------------------
    nRetCode = MySQL_Query(pSrcHandle, szSQL);
    KGLOG_PROCESS_ERROR(nRetCode);

    pSrcResultSet = mysql_store_result(pSrcHandle);
    KGLOG_PROCESS_ERROR(pSrcResultSet);

    uFieldCount = mysql_num_fields(pSrcResultSet);    
    KGLOG_PROCESS_ERROR(uFieldCount == 2);

    SrcRow = mysql_fetch_row(pSrcResultSet);
    KGLOG_PROCESS_ERROR(SrcRow);

    pcszSrcCharset = (const char*)SrcRow[1];
    KGLOG_PROCESS_ERROR(pcszSrcCharset);

    // ---------------- destination DB--------------------
    nRetCode = MySQL_Query(pDstHandle, szSQL);
    KGLOG_PROCESS_ERROR(nRetCode);

    pDstResultSet = mysql_store_result(pDstHandle);
    KGLOG_PROCESS_ERROR(pDstResultSet);

    uFieldCount = mysql_num_fields(pDstResultSet);    
    KGLOG_PROCESS_ERROR(uFieldCount == 2);

    DstRow = mysql_fetch_row(pDstResultSet);
    KGLOG_PROCESS_ERROR(DstRow);

    pcszDstCharset = (const char*)DstRow[1];
    KGLOG_PROCESS_ERROR(pcszDstCharset);

    // ----------------------------------------------------

    nRetCode = strcmp(pcszSrcCharset, pcszDstCharset);
    if (nRetCode != 0)
    {
        KGLogPrintf(
            KGLOG_ERR, 
            "Database charset not equal, SourceDB(%s) , Destination(%s)", 
            pcszSrcCharset, pcszDstCharset
        );
...
Exit0:
    if (pSrcResultSet)
    {
        mysql_free_result(pSrcResultSet);
        pSrcResultSet = NULL;
    }
    if (pDstResultSet)
    {
        mysql_free_result(pDstResultSet);
        pDstResultSet = NULL;
    }
    return bResult;
}
