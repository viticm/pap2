...
HRESULT ChangePathExtName(
    ...
    unsigned uDestPathNameBufferSize, 
...
)
{
    ...
    KGLOG_PROCESS_ERROR(uDestPathNameBufferSize > (uNoExtNameLen +  sizeof('.') + uNewExtNameLen));
    
    memcpy(pszRetDestPathName, cszSourcePathName, uNoExtNameLen);
    pszRetDestPathName[uNoExtNameLen] = '.';
    memcpy(pszRetDestPathName + uNoExtNameLen + 1, cszNewExtName, uNewExtNameLen + sizeof('\0'));
