...
//posSide 表示是否相交正面
//negSide 表示是否相交背面
BOOL IsLineIntersectTriangle(D3DXVECTOR3& LineA,D3DXVECTOR3& LineDirection,D3DXVECTOR3& a, D3DXVECTOR3& b, D3DXVECTOR3& c, D3DXVECTOR3& TriangleNormal, BOOL posSide, BOOL negSide, float& dist)
{
	float t;
	{
		float denom = D3DXVec3Dot(&LineDirection,&TriangleNormal);
		//检查相交平面
		if(denom>+0.0001)
		{
			if(!negSide)
		...
		t = D3DXVec3Dot(&TriangleNormal,&(a - LineA));
		if(t<0)
		{
			dist = 0;
			return FALSE;
	...
	size_t i0, i1;
	{
		float n0 = abs(TriangleNormal.x);
		float n1 = abs(TriangleNormal.y);
		float n2 = abs(TriangleNormal.z);
		i0=1;i1=2;
		if(n1>n2)
		{
			if(n1>n0) i0 = 0;
	...
	float u2 = c[i0] - a[i0];
	float v2 = c[i1] - a[i1];
	float u0 = t * LineDirection[i0] + LineA[i0] - a[i0];
	float v0 = t * LineDirection[i1] + LineA[i1] - a[i1];
...
BOOL IsRayIntersectSphere(D3DXVECTOR3* pIntersection,
								D3DXVECTOR3* pS_Center,float S_Radius,
								D3DXVECTOR3* pLineA,D3DXVECTOR3* pDir)
{
	D3DXPLANE plane;
	D3DXPlaneFromPointNormal(&plane,pS_Center,pDir);
	D3DXVECTOR3 Inter;
	D3DXVECTOR3 vB = *pLineA + *pDir * 50000;

	if (D3DXPlaneIntersectLine(&Inter,&plane,pLineA,&vB)==NULL)
		return FALSE;
...
void MatrixExractNew(D3DXVECTOR3& Scaling,D3DXVECTOR3& Position,
				  D3DXMATRIX& Rotation,D3DXMATRIX* pInMatrix)
{
	D3DXVECTOR3 X(1,0,0);
	D3DXVECTOR3 Y(0,1,0);
	...
	D3DXVec3TransformNormal(&X,&X,pInMatrix);
	D3DXVec3TransformNormal(&Y,&Y,pInMatrix);
	D3DXVec3TransformNormal(&Z,&Z,pInMatrix);
	...
	Rotation._11 = X.x;Rotation._21 = X.y;Rotation._31 = X.z;
	Rotation._12 = Y.x;Rotation._22 = Y.y;Rotation._32 = Y.z;
	Rotation._13 = Z.x;Rotation._23 = Z.y;Rotation._33 = Z.z;
	...
	D3DXMATRIX matINV,matScale;
	//D3DXMatrixScaling(&matScale,Sx,Sy,Sz);
	D3DXMatrixInverse(&matINV,NULL,&Rotation);
	D3DXMatrixMultiply(&matScale,&SrcMatrix,&matINV);

	D3DXVECTOR3 A,B,C;
	A = D3DXVECTOR3(matScale._11,matScale._12,matScale._13);
	B = D3DXVECTOR3(matScale._21,matScale._22,matScale._23);
	C = D3DXVECTOR3(matScale._31,matScale._32,matScale._33);
	float Sx,Sy,Sz;
	Sx = D3DXVec3Length(&A);
	Sy = D3DXVec3Length(&B);
	Sz = D3DXVec3Length(&C);
...
FLOAT PointDistanceLine( const D3DXVECTOR3& vP, const D3DXVECTOR3& vSrc, const D3DXVECTOR3& vNormalizedDir )
{
	//用在计算机图形学-几何工具算法详解里面抄的一个算法来解决这个问题，不用倒腾Plane了
	//不过实际和上面的没有啥不一样的
	//两点式有很多坏处，要检查是否两点接近之类的，用d3dx的函数也只不过是隐藏或者忽略了这个行为而已
	D3DXVECTOR3 vIntersectionPoint = PointDistanceToLine(vP, vSrc, vNormalizedDir);
	return D3DXVec3Length(&(vP - vIntersectionPoint));
}
...
inline bool IsVertexInSphere(const D3DXVECTOR3& vCenter,
                             float        fRadius,
...
inline bool IsLineCrossSphere(const D3DXVECTOR3& vCenter,
                              float        fRadius,
                              const D3DXVECTOR3& vStr,
...
inline bool IsVertexInTriangle(const D3DXVECTOR3& vVertex,
                               const D3DXVECTOR3& vTriA,
...
bool IsSphereCrossTriangle(const D3DXVECTOR3& vCenter, float fRadius, const D3DXVECTOR3& vTriA, const D3DXVECTOR3& vTriB, const D3DXVECTOR3& vTriC, bool bCW, ObstructdInfo* pInfo)
{
    ASSERT(pInfo);
...
bool IsSphereCrossTriangle(const D3DXVECTOR3& vCenter, float fRadius, const D3DXVECTOR3& vTriA, const D3DXVECTOR3& vTriB, const D3DXVECTOR3& vTriC)
{
    D3DXVECTOR3 normal;
    D3DXPLANE   panel;
    D3DXVECTOR3 vDest;
    ...
    D3DXVECTOR3 vIst2Cer  = vCenter - vInst;
    float       fDistanceSq = D3DXVec3LengthSq(&vIst2Cer);
...
namespace {
    ...
    __forceinline int planeBoxOverlap(float normal[3],float d, float maxbox[3])
    {
        int q;
        float vmin[3],vmax[3];
        for(q=0;q<=2;q++)
    ...
    __forceinline int _IsTriangleBoxOverlap(float boxcenter[3],float boxhalfsize[3],float triverts[3][3])
    {
        ...
        float _min,_max,d,p0,p1,p2,rad,fex,fey,fez;  
        ...
        /* test in 0-direction */
        FINDMINMAX(v0[0],v1[0],v2[0],_min,_max);
        if(_min>boxhalfsize[0] || _max<-boxhalfsize[0]) return 0;

        /* test in 1-direction */
        FINDMINMAX(v0[1],v1[1],v2[1],_min,_max);
        if(_min>boxhalfsize[1] || _max<-boxhalfsize[1]) return 0;

        /* test in 2-direction */
        FINDMINMAX(v0[2],v1[2],v2[2],_min,_max);
        if(_min>boxhalfsize[2] || _max<-boxhalfsize[2]) return 0;
...
{
    float c[3] = { boxCenter.x, boxCenter.y, boxCenter.z };
    float s[3] = { boxHalfSize.x, boxHalfSize.y, boxHalfSize.z };
    float t[3][3] = {
        { tri[0].x, tri[0].y, tri[0].z },
        { tri[1].x, tri[1].y, tri[1].z },
        { tri[2].x, tri[2].y, tri[2].z }
    };
    return !!_IsTriangleBoxOverlap(c, s, t);
}